...
for i_episode in range(MAX_EXE_NUM):
    ...
    if (i_episode + 1) % UPDATE_CYCLE == 0 and agent.buffer.get_buffer_size() > 0:
        ...
        for key, value in train_info.items():
            writer.add_scalar(f"Train/{key}", value, i_episode)
        print("--- 训练结束 ---\n")
        ...
        with torch.no_grad():
            ...
            writer.add_scalar('Eval/Reward_Sum', eval_reward_sum, i_episode)
        print("--- 评估结束 ---\n")
