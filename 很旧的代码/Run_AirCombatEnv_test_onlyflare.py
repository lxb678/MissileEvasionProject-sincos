    ...
    with torch.no_grad():
        ...
        while (not done_eval):
            if t % (round(env.dt_dec/env.dt_normal)) == 0:
                ...
                # action_list.append(np.array([round(env.t_now, 2), action_eval[0]]))
...
episode_times_ms = np.array(episode_times) * 1000  # convert to milliseconds
x = np.arange(1, len(episode_times_ms) + 1)
y = episode_times_ms

plt.figure(figsize=(16, 6))
plt.style.use('seaborn-v0_8-muted')  # set style

# Scatter plot with larger points and clear edges
plt.scatter(x, y, c='dodgerblue', edgecolors='k', s=60, alpha=0.8, label='Simulation Time')

# Add mean line
mean_y = np.mean(y)
plt.axhline(mean_y, color='orange', linestyle='--', linewidth=2, label=f'Mean Simulation Time: {mean_y:.1f} ms')

# Set title and labels
# plt.title('Scatter Plot of Strategy Generation Time', fontsize=16, fontweight='bold')
