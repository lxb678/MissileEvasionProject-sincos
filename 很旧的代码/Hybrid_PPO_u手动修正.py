...
# --- 动作空间配置 ---
# 定义连续和离散动作的维度
CONTINUOUS_DIM = 3  # 连续动作的数量 (例如: nx, nz, p_cmd)
DISCRETE_DIM = 1    # 离散动作的数量 (例如: flare)
# 定义连续动作的键名，用于后续的动作缩放
CONTINUOUS_ACTION_KEYS = ['nx', 'nz', 'p_cmd']

# --- 动作范围定义 ---
# 定义每个动作的物理范围，用于将神经网络输出的 [-1, 1] 范围映射到实际环境中
ACTION_RANGES = {
    'nx':       {'low': -1.0, 'high': 1.0}, # 切向过载指令范围
    'nz':       {'low': -5.0, 'high': 9.0},  # 法向过载指令范围
    # 'phi_cmd':  {'low': -np.pi, 'high': np.pi},   # 假设滚转指令是-180到+180度
    # 假设滚转角速度范围是 -240度/秒 到 +240度/秒  这个值需要根据你的飞机模型进行调整
    'p_cmd':    {'low': -4.0 * np.pi / 3.0, 'high': 4.0 * np.pi / 3.0}, # 滚转角速度指令范围
...
class Actor(Module):
   ...
   """
    def __init__(self):
        ...
        self.log_std_max =  2.0  # (中文) 关键修改：从 2.0 降低到 0.0
        # 初始化网络模型、优化器和学习率调度器
        self.init_model()
   ...
   """
    ...
    def choose_action(self, state, deterministic=False):
        ...
        with torch.no_grad():
            ...
            # 4. 手动计算 a 的对数概率 log p(a)
            # 根据变量变换法则: log p(a) = log p(u) - log |da/du|
            # 4.1 基础正态分布的 log_prob
            log_prob_u = continuous_base_dist.log_prob(u).sum(dim=-1)
            # 4.2 计算雅可比修正项 log(1 - tanh(u)^2) 的稳定形式
            # 公式: log p(a) = log p(u) - log |da/du|
            # log |da/du| = 2 * (log(2) - u - softplus(-2u))
            jacobian_correction = (2 * (math.log(2.) - u - F.softplus(-2. * u))).sum(dim=-1)
            # 4.3 得到最终的对数概率
            log_prob_cont = log_prob_u - jacobian_correction
    ...
    def learn(self):
        ...
        for _ in range(self.ppo_epoch):
            ...
            for batch in self.buffer.generate_batches():
                ...
                # 5. 再次使用 Tanh-Normal 的手动计算流程来得到新策略下的 log_prob
                new_log_prob_u = new_cont_base_dist.log_prob(u_from_buffer).sum(dim=-1)
                new_jacobian_correction = (2 * (math.log(2.) - u_from_buffer - F.softplus(-2. * u_from_buffer))).sum(dim=-1)
                new_log_prob_cont = new_log_prob_u - new_jacobian_correction
                ...
                old_value_from_buffer = check(values[batch]).to(**CRITIC_PARA.tpdv)
