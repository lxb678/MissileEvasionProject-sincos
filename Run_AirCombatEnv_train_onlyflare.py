...
#记录训练的损失等数值，用于绘制图表  使用tensorboard --logdir=log 路径 的命令绘制 文件名是随机种子-训练日期-是否使用储存的模型
    ...
    for t in range(10000):
        ...
        if t % (round(env.dt_dec/env.dt_normal)) == 0:
            ...
            with torch.no_grad():   #交互时梯度不回传
                ...
                # 只输出红外干扰弹，补齐激光定向干扰，可减少对环境的更改
                # action = np.append(action, 0)
                # action_list.append(np.array([round(env.t_now, 2), action[0], action[1]]))
                action_list.append(np.concatenate(([round(env.t_now, 2)], action)))
                prob = prob.cpu().detach().numpy()
                value = agent.get_value(observation).cpu().detach().numpy()
                state = observation

            observation, reward, done, reward_4,_ = env.step(np.array([action[0], 0]))
            # rewards = reward
        ...
        with torch.no_grad():
            ...
            for t in range(10000):
                if t % (round(env.dt_dec/env.dt_normal)) == 0:
                    ...
                    action_eval = np.append(action_eval, 0) # 补齐激光定向干扰，可减少对环境的更改

                    # reward = (reward_eval + reward_4 + 10.0) / 20.0
                    # reward_sum += reward
                    reward_sum += reward_eval + reward_4

                    observation_eval, reward_eval, done_eval, reward_4, _ = env.step(np.array([action_eval[0], 0]))
