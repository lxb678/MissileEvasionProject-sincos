...
# ==============================================================================
# <<< 修改 >>>: 定义新的基于 GRU + Attention 的 Actor 和 Critic
...
class Actor_GRU(Module):
    """
    Actor 网络 (策略网络) - [GRU -> 时序Attention -> MLP -> 动作头]

    该架构的核心思想是两阶段处理：
    1. 时序建模阶段（时间维度）：GRU首先直接处理原始的状态序列，捕捉状态随时间变化的
       基本动态和趋势，并输出一个包含历史上下文的隐藏状态序列。
    2. 上下文增强阶段（时间维度）：自注意力机制接着处理这个隐藏状态序列，通过计算
       序列中不同时间步之间的相互关系，对每个时间步的表示进行加权和增强，
       从而让模型能够聚焦于对当前决策最重要的历史信息。
    """
    ...
    def __init__(self, dropout_rate=0.05, weight_decay=1e-4):
        ...
        # <<< 新增 1. GRU 时序建模层 >>>
        # 作用：直接处理原始输入的状态序列 (B, S, D)，捕捉时间依赖性。
        self.gru = GRU(self.input_dim, self.rnn_hidden_size, batch_first=True)

        # <<< 新增 2. 时序自注意力层 (Temporal Self-Attention) >>>
        # 作用：在GRU输出的隐藏状态序列上进行交互，计算不同时间步之间的重要性。
        # embed_dim 是GRU的输出维度 rnn_hidden_size。
        assert self.rnn_hidden_size % ATTN_NUM_HEADS == 0, "rnn_hidden_size must be divisible by num_heads"
        self.attention = MultiheadAttention(embed_dim=self.rnn_hidden_size,
        ...
        # --- 定义两个LayerNorm ---
        self.attention_layernorm = LayerNorm(self.rnn_hidden_size)  # 这个作为 Pre-LN
        self.post_layernorm = LayerNorm(self.rnn_hidden_size)  # <<< ✅ 新增这个作为 Post-LN >>>

        # 3. 共享MLP骨干网络
        # 作用：对经过Attention增强的时序特征进行进一步的非线性变换。
        # 输入维度是 rnn_hidden_size，因为Attention层不改变维度。
    ...
    def forward(self, obs, hidden_state):
        ...
        # --- 阶段二：上下文增强 (时序注意力) ---
        # 1. Pre-LayerNorm: 在送入注意力层前进行归一化。
        normed_gru_out = self.attention_layernorm(gru_out)

        # 2. Multi-head Self-attention: 对GRU输出的序列进行加权。
        attn_out, _ = self.attention(normed_gru_out, normed_gru_out, normed_gru_out)
        attn_out = self.attn_dropout(attn_out)

        # 3. 残差连接 + Post-LayerNorm
        contextualized_sequence = gru_out + attn_out
        contextualized_sequence = self.post_layernorm(contextualized_sequence)

        # --- 阶段三：决策 ---
        # 4. MLP加工: 对经过Attention增强的序列进行深度加工。
        mlp_output = self.shared_network(contextualized_sequence)

        # 如果是单步推理，移除序列维度。
        final_features = mlp_output
        if not is_sequence:
            final_features = final_features.squeeze(1)  # (B, 1, mlp_dim) -> (B, mlp_dim)
...
class Critic_GRU(Module):
    """
        Critic 网络 (价值网络) - 采用与 Actor 完全相同的 [GRU -> 时序Attention -> MLP] 结构。
    ...
    def __init__(self, dropout_rate=0.05, weight_decay=1e-4):
        ...
        self.attention = MultiheadAttention(embed_dim=self.rnn_hidden_size,
                                            ...
                                            batch_first=True)
        self.attn_dropout = Dropout(p=dropout_rate)
        self.attention_layernorm = LayerNorm(self.rnn_hidden_size)  # Pre-LN
        self.post_layernorm = LayerNorm(self.rnn_hidden_size)  # Post-LN
    ...
    def forward(self, obs, hidden_state):
        ...
        # --- 阶段三：价值评估 ---
        # MLP 加工并输出价值
        base_features_sequence = self.network_base(contextualized_sequence)
