...
ATTN_NUM_HEADS = 2 #1 #3 #4 #8 #4 #1 #2       # <<< 这是您的关键修改
...
class Actor_GRU(Module):
    """
    Actor 网络 (策略网络) - [GRU -> 特征级 Attention -> MLP -> 动作头]

    该架构的核心思想是两阶段处理：
    1. 时序建模阶段（时间维度）：首先使用 GRU 处理整个输入序列，在每个时间步 `t`
       生成一个包含了历史信息的隐藏状态向量 `h_t`。
    ...
    def __init__(self, dropout_rate=0.05, weight_decay=1e-4):
        ...
        self.rnn_hidden_size = self.input_dim
        self.attn_embedding_dim = 64 #64  # n1
        self.weight_decay = weight_decay

        # --- 模块定义 ---

        # 1. GRU 时序建模层
        # 作用：处理原始输入序列 (B, S, D)，捕捉时间依赖性。
        ...
        self.attention = MultiheadAttention(embed_dim=self.attn_embedding_dim,
                                            ...
                                            dropout=0.0,
                                            batch_first=True)
        # self.attention_layernorm = LayerNorm(self.attn_embedding_dim)

        # self.pooling_query = torch.nn.Parameter(torch.randn(1, self.attn_embedding_dim))
        # self.pooling_softmax = torch.nn.Softmax(dim=1)

        # 4. 共享MLP骨干网络
        # 作用：对注意力模块输出的特征进行进一步的非线性变换。
        # 输入维度是 attn_embedding_dim，因为池化后的向量维度与嵌入维度相同。
        # ========================= [💥 关键修正 💥] =========================
        # 输入维度必须是展平后的维度: rnn_hidden_size * attn_embedding_dim
    ...
    def forward(self, obs, hidden_state):
        ...
        # normed_tokens = token_embeds
        attn_out, _ = self.attention(token_embeds, token_embeds, token_embeds)
        token_context = token_embeds + attn_out  # 残差连接, shape: (B*S, G, N1)

        # # 5. 池化: 将 G 个交互后的特征 Token 聚合成一个单一向量
        # #    (B*S, G, N1) -> (B*S, N1)
        # pooled_context = token_context.mean(dim=1)
        # # # --- 新的注意力池化 ---
        # # # token_context 的形状是 (B*S, D, embedding_dim)
        # # # self.pooling_query 的形状是 (1, embedding_dim)
        # #
        # # # 1. 计算相似度分数 (使用点积)
        # # #    (B*S, D, emb) @ (emb, 1) -> (B*S, D, 1)
        # # attention_scores = torch.matmul(token_context, self.pooling_query.t())
        # #
        # # # 2. 将分数转换为权重
        # # #    在特征维度 D 上进行 softmax
        # # attention_weights = self.pooling_softmax(attention_scores)  # 形状: (B*S, D, 1)
        # #
        # # # 3. 加权求和
        # # #    (B*S, D, emb) * (B*S, D, 1) -> (B*S, D, emb)  (逐元素相乘，广播机制)
        # # #    然后求和
        # # pooled_context = (token_context * attention_weights).sum(dim=1)  # 形状: (B*S, embedding_dim)
        ...
        # # 6. 恢复序列结构: (B*S, G, N1) -> (B, S, N1)
        # contextualized_sequence = token_context.view(B, S, N1)
...
class Critic_GRU(Module):
    """
        Critic 网络 (价值网络) - 采用与 Actor 完全相同的 [GRU -> 特征级 Attention -> MLP] 结构。
        """
    ...
    def __init__(self, dropout_rate=0.05, weight_decay=1e-4):
        ...
        # self.rnn_hidden_size = RNN_HIDDEN_SIZE  # G
        self.rnn_hidden_size = self.input_dim
        # self.attn_embedding_dim = 128 #64  # n1
        # self.weight_decay = weight_decay
        # --- 模块定义 (与 Actor 结构相同) ---
        ...
        #
        # # 3. 特征级自注意力层
        # assert self.attn_embedding_dim % ATTN_NUM_HEADS == 0
        # self.attention = MultiheadAttention(embed_dim=self.attn_embedding_dim,
        #                                     num_heads=ATTN_NUM_HEADS,
        #                                     dropout=0.0,  # Critic可以用稍高的dropout
        #                                     batch_first=True)
        # self.attention_layernorm = LayerNorm(self.attn_embedding_dim)
        #
        # self.pooling_query = torch.nn.Parameter(torch.randn(1, self.attn_embedding_dim))
    ...
    def forward(self, obs, hidden_state):
        ...
        # # pooled_context = (token_context * attention_weights).sum(dim=1)  # 形状: (B*S, embedding_dim)
        # # 5. Reshape
        # contextualized_sequence = pooled_context.view(B, S, N1)
