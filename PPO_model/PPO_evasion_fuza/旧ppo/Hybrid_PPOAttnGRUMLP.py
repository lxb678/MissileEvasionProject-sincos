...
ATTN_NUM_HEADS = 2 #2 #3 #4 #8 #4 #1 #2       # <<< 这是您的关键修改
...
class Actor_GRU(Module):
    ...
    [特征嵌入 -> 原始特征级 Attention -> GRU -> MLP -> 动作头]

    架构特点：
    - 特征嵌入：自注意力在D个特征的高维嵌入上操作，表达能力更强。
    - 无池化瓶颈：注意力输出的D个上下文感知特征被重塑为一个大的D*embed_dim维向量，
                  完整地送入GRU，保留了所有特征信息。
    - 多头注意力：由于嵌入维度 > 1，现在可以使用多头注意力。
    """

    def __init__(self, dropout_rate=0.05, weight_decay=1e-4):
        super(Actor_GRU, self).__init__()
        # --- 基础参数定义 ---
        self.input_dim = ACTOR_PARA.input_dim  # D
        self.log_std_min = -20.0
        self.log_std_max = 2.0
        self.rnn_hidden_size = RNN_HIDDEN_SIZE
        self.weight_decay = weight_decay
        self.feature_dim_for_attn = 1  # 每个特征是1维标量

        # <<< MODIFIED 1/6 >>>: 定义一个新的嵌入维度
        self.embedding_dim = 64  # 每个特征将被映射到这个维度

        # <<< MODIFIED 2/6 >>>: 检查多头注意力的约束
        # 现在 embed_dim > 1，可以重新使用多头注意力了
        ...
        # 2. GRU 时序建模层
        # <<< 修改 2/3 >>>: GRU的输入维度现在是 D (self.input_dim)，因为它接收重塑后的完整特征向量
    ...
    def forward(self, obs, hidden_state):
        ...
        token_embeds = self.feature_embed(feat_tokens)  # 形状: (B*S, D, embedding_dim)
        # 自注意力计算 (不变)
        # normed_tokens = self.attention_layernorm(token_embeds)
        # normed_tokens = feat_tokens
        ...
        # attention_weights = self.pooling_softmax(attention_scores)
        # pooled_context = (token_context * attention_weights).sum(dim=1) # 原输出 (B*S, 1)

        # # 5. 重塑以保留D维信息
        # # (B*S, D, 1) -> (B*S, D) -> (B, S, D)
        # contextualized_sequence = token_context.squeeze(-1).view(B, S, D)

        # <<< MODIFIED 6/6, part 2 >>>: 重塑以保留所有信息
        # (B*S, D, embedding_dim) -> (B*S, D * embedding_dim)
...
class Critic_GRU(Module):
    """
    Critic 网络 (价值网络) - 采用与 Actor 完全相同的 [无池化瓶颈] 结构。
    """
    ...
    def __init__(self, dropout_rate=0.05, weight_decay=1e-4):
        ...
        self.rnn_hidden_size = RNN_HIDDEN_SIZE
        self.weight_decay = weight_decay
        # self.feature_dim_for_attn = 1
        #
        # _ATTN_NUM_HEADS = 1
        # if ATTN_NUM_HEADS != 1:
        #     print(f"[警告] Critic架构修改强制 ATTN_NUM_HEADS=1 (原设置为 {ATTN_NUM_HEADS})")

        # --- 模块定义 ---
        # # 1. 特征级自注意力层
        # self.attention = MultiheadAttention(embed_dim=self.feature_dim_for_attn,
        #                                     num_heads=_ATTN_NUM_HEADS,
        #                                     dropout=0.0,
        #                                     batch_first=True)
        # self.attention_layernorm = LayerNorm(self.feature_dim_for_attn)
        ...
        # 2. GRU 时序建模层
        # <<< 修改 2/3 >>>: GRU的输入维度现在是 D (self.input_dim)
        self.gru = GRU(self.input_dim, self.rnn_hidden_size, batch_first=True)
    ...
    def forward(self, obs, hidden_state):
        ...
        # feat_tokens = obs_tensor.contiguous().view(B * S, D, 1)
        #
        # normed_tokens = self.attention_layernorm(feat_tokens)
        # attn_out, _ = self.attention(normed_tokens, normed_tokens, normed_tokens)
        # token_context = feat_tokens + attn_out  # 形状: (B*S, D, 1)
        #
        # # <<< 修改 3/3 >>>: 移除池化，改为重塑
        # contextualized_sequence = token_context.squeeze(-1).view(B, S, D)
...
#         self.rnn_hidden_size = RNN_HIDDEN_SIZE
#         # 定义注意力机制和GRU的工作维度
#         self.embedding_dim = 1 #64#128 #RNN_HIDDEN_SIZE
#         # self.rnn_hidden_size = self.embedding_dim
...
#
#         self.pooling_query = torch.nn.Parameter(torch.randn(1, self.embedding_dim))
#         self.pooling_softmax = torch.nn.Softmax(dim=1)
#
#         # 3. GRU 时序建模层
...
#
#         # # --- 初始化与优化器 ---
#         # # 对非输出层应用 Kaiming/Orthogonal 初始化
#         # self.apply(init_weights)
#         #
#         # # <<< ✅ 修改：对输出层使用 Xavier 初始化，而不是固定范围的 uniform >>>
#         # # 这种方法更通用，对不同的网络尺寸更鲁棒
...
#         # --- 阶段三：决策 ---
#         # # 7. 准备MLP输入
#         # # new_hidden 的形状是 (1, B, rnn_hidden_size)，代表最终隐藏状态。
#         # # 7a. 移除多余的层维度: (1, B, H) -> (B, H)
#         # final_hidden_state = new_hidden.squeeze(0)
#         # # 7b. 扩展以匹配序列长度: (B, H) -> (B, 1, H) -> (B, S, H)
#         # # 这一步是关键：我们将同一个全局信息（最终隐藏状态）复制S次，
#         # # 作为序列中每一个时间步的输入特征。
#         # mlp_input = final_hidden_state.unsqueeze(1).expand(-1, S, -1)
#
#         # 8. MLP加工: 对全局摘要特征进行深度加工
#         mlp_output = self.shared_network(gru_out)
#
#         # 如果是单步推理，移除序列维度。
#         final_features = mlp_output
...
#
#         # 在 forward 中
#         # 原来的池化: pooled_context = token_context.mean(dim=1)
#
#         # --- 新的注意力池化 ---
#         # token_context 的形状是 (B*S, D, embedding_dim)
#         # self.pooling_query 的形状是 (1, embedding_dim)
