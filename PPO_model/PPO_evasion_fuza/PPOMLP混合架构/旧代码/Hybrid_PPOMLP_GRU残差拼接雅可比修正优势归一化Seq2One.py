...
RNN_HIDDEN_SIZE =  128 #64  #128 #64 #128 #64 #9 #9 #32 #9  # GRU 层的隐藏单元数量
...
class PPO_continuous(object):
    ...
    def learn(self):
        """
        执行 PPO 的学习和更新步骤。
        已集成：Seq2One, 优势归一化, 雅可比修正, 以及 [新增] KL散度早停。
        """
        ...
        train_info = {'critic_loss': [], 'actor_loss': [], 'dist_entropy': [], 'entropy_cont': [], 'adv_targ': [],
                      'ratio': [], 'approx_kl': []}  # <<<< 新增 approx_kl

        # =================================================================
        # [新增 1] 设定目标 KL 散度阈值
        # 如果一次更新导致策略差异超过这个值，说明隐藏状态过期严重，必须停止
        target_kl = 0.02
        # =================================================================

        # 2. PPO 训练循环
        for epoch_i in range(self.ppo_epoch):  # 将 _ 改为 epoch_i 方便打印日志
            if self.use_rnn:
            ...
            for batch_data in batch_generator:
                ...
                if self.use_rnn:
                    ...
                    target_return = return_
                    target_old_prob = old_prob
                ...
                # =================================================================
                # [新增 2] 计算当前 Batch 的 KL 散度
                # =================================================================
                with torch.no_grad():
                    # Approx KL: (old_log_prob - new_log_prob)
                    # 注意：target_old_prob 是旧策略的 log_prob
                    batch_kl = (target_old_prob - current_prob).mean().item()
                    epoch_kls.append(batch_kl)
                    train_info['approx_kl'].append(batch_kl)
                # =================================================================
                ...
                # 5. Critic 更新
                if self.use_rnn:
                    new_value_seq, _ = self.Critic(state, initial_critic_h)
            ...
            # =================================================================
            # [新增 3] Epoch 结束后的早停检查 (Early Stopping)
            # =================================================================
            avg_kl = np.mean(epoch_kls)
            if avg_kl > target_kl * 1.5:
                print(
                    f"  [PPO Early Stop] Epoch {epoch_i + 1}/{self.ppo_epoch} stopped. Avg KL ({avg_kl:.4f}) > {target_kl * 1.5:.4f}")
                break
            # =================================================================
    ...
    #                 # =========================================================
    #                 # advantage shape: [Batch, Seq, 1] -> [Batch, 1]
    #                 target_advantage = advantage[:, -1, :]
    #                 # return_ shape: [Batch, Seq, 1] -> [Batch, 1]
    #                 target_return = return_[:, -1, :]
    #                 # old_prob shape: [Batch, Seq] -> [Batch]
    #                 target_old_prob = old_prob[:, -1]
    #
    #             else:
    #                 # MLP 模式逻辑保持不变
    ...
    #
    #             # ================= [雅可比修正与 LogProb 计算] =================
    #             # 此时所有的 LogProb 和 Entropy 维度都是 [Batch, Seq] (如果是RNN)
    #
    #             # --- A. 连续动作 Log Prob ---
    #             log_prob_u_buffer = new_dists['continuous'].log_prob(u_from_buffer).sum(dim=-1)
    #             correction_buffer = 2.0 * (np.log(2.0) - u_from_buffer - F.softplus(-2.0 * u_from_buffer)).sum(dim=-1)
    ...
    #             new_prob_seq = new_log_prob_cont + new_log_prob_disc
    #
    #             # 🔥 [修正 Bug] 这里不要先求 mean()，要保持 [Batch, Seq] 维度
    #             total_entropy_seq = entropy_cont + entropy_disc
    #
    #             # =========================================================
    ...
    #             train_info['critic_loss'].append(critic_loss.item())
    #             train_info['actor_loss'].append(actor_loss.item())
    #             train_info['dist_entropy'].append(current_entropy.mean().item())  # 记录当前步的熵
    #             train_info['entropy_cont'].append(entropy_cont.mean().item())  # 记录整体连续熵(参考用)
    #             train_info['adv_targ'].append(target_advantage.mean().item())
    #             train_info['ratio'].append(ratio.mean().item())
    #
    #     # 6. 清理
    ...
    #
    #                 # =========================================================
    #                 # 🔥 [修改点 1] Seq2One 核心：截取 Target 的最后一步
    #                 # =========================================================
    #                 # 我们依然保留 action_batch 为全序列，因为计算 log_prob 需要它
    #                 # 但我们需要取出用于算 Loss 的“最后一步”的目标值
    #
    ...
    #
    #             # =========================================================
    #             # 🔥 [修改点 2] Seq2One 核心：截取 Prediction 的最后一步
    #             # =========================================================
    #             if self.use_rnn:
    #                 # 只取序列最后一步的 LogProb 和 熵
    #                 # [Batch, Seq] -> [Batch]
    #                 current_prob = new_prob[:, -1]
    #                 current_entropy = total_entropy_val[:, -1]
    #             else:
    #                 current_prob = new_prob
    #                 current_entropy = total_entropy_val
    #
    #             # 计算 Ratio (使用截取后的 prob)
    #             # target_old_prob 也是截取后的
    #             log_ratio = current_prob - target_old_prob
    #             ratio = torch.exp(torch.clamp(log_ratio, -20.0, 20.0))
    #             # 计算 Actor Loss (使用截取后的 advantage)
    #             # 确保维度匹配
    #             if target_advantage.dim() > ratio.dim():
    #                 target_advantage = target_advantage.squeeze(-1)
    #
    #             surr1 = ratio * target_advantage
    #             surr2 = torch.clamp(ratio, 1.0 - AGENTPARA.epsilon, 1.0 + AGENTPARA.epsilon) * target_advantage
    #
    #             # Loss = -Policy_Loss - Entropy_Bonus
    #             # current_entropy.mean() 取的是 Batch 维度的平均
    #             actor_loss = -torch.min(surr1, surr2).mean() - AGENTPARA.entropy * current_entropy.mean()
    #
    #             # # 计算 Ratio
    ...
    #             #
    #             # # Loss = -Policy_Loss - Entropy_Bonus
    #             # actor_loss = -torch.min(surr1, surr2).mean() - AGENTPARA.entropy * total_entropy_val
    #
    #             # 更新 Actor
    #             self.Actor.optim.zero_grad()
    #             actor_loss.backward()
    ...
    #                 new_value_seq, _ = self.Critic(state, initial_critic_h)
    #
    #                 # 🔥 [修改点 3] Seq2One 核心：截取 Value 的最后一步
    #                 # [Batch, Seq, 1] -> [Batch, 1]
    #                 new_value = new_value_seq[:, -1, :]
    #             else:
    #                 new_value = self.Critic(state)
    ...
    #                 new_value = new_value.unsqueeze(-1)
    #
    #                 # 计算 Critic Loss (使用截取后的 target_return)
    #             critic_loss = torch.nn.functional.mse_loss(new_value, target_return)
    #
    #             # 更新 Critic
    #             self.Critic.optim.zero_grad()
    ...
    #             train_info['critic_loss'].append(critic_loss.item())
    #             train_info['actor_loss'].append(actor_loss.item())
    #             train_info['dist_entropy'].append(total_entropy_val.item())
    #             train_info['entropy_cont'].append(entropy_cont.mean().item())
