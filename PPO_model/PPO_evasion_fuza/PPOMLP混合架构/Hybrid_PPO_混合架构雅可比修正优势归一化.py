...
class Actor(Module):
    ...
    def _init_weights(self):
        ...
        nn.init.orthogonal_(self.continuous_head.weight, gain=0.01)
        nn.init.constant_(self.continuous_head.bias, 0)
...
class Critic(Module):
    ...
    def _init_weights(self):
        ...
        # 修改说明：使用 [-1] 索引来获取 Sequential 中的最后一个模块 (即 fc_out)
        output_layer = self.network[-1]

        # 确保最后一层确实是线性层（安全检查，可不写）
