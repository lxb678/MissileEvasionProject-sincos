...
class Buffer(object):
    ...
    def clear_memory(self):
        """清空所有存储的经验"""
        self.states = []
        self.global_states = []  # <<< 新增：保存Critic上帝视角
        self.actions = []
        self.probs = []
        self.values = []
        self.rewards = []
    ...
    def store_transition(self, state, global_state, value, action, probs, reward, done,
        ...
        self.global_states.append(global_state)  # <<< 新增
    ...
    def get_all_data(self):
        ...
        g = np.array(self.global_states, dtype=np.float32)  # <<< 新增
        ...
        return s,g, v, a, p, r, d, actor_h, critic_h, attn_w
    ...
    def generate_sequence_batches(self, sequence_length, batch_size, advantages, returns, verbose=False):
        ...
        global_states_np = np.array(self.global_states, dtype=np.float32)  # <<< 新增
        ...
        for i in range(n_full_batches):
            ...
            # 初始化 Batch 容器
            b_states,b_global_states, b_actions, b_probs = [], [],[], []
            b_adv, b_ret = [], []
            b_values = []
            ...
            for start_idx in batch_start_indices:
                ...
                b_global_states.append(global_states_np[start_idx:end_idx])  # <<< 新增同等切片
            ...
            stacked_global_states = np.stack(b_global_states)  # <<< 新增
