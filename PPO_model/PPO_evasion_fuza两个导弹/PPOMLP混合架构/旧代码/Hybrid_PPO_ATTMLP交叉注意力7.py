...
# ==============================================================================
#           <<< 新增辅助类: 实体编码器 (Deep Entity Encoder) >>>
# ==============================================================================

class EntityEncoder(nn.Module):
    """
    用于将原始观测值 (4维或7维) 映射到高维 Embedding (64维)。
    相比简单的 Linear 层，这里使用了 MLP + LayerNorm，能更好地提取非线性特征。
    """
    ...
    def __init__(self, input_dim, output_dim, hidden_dim=128):
        ...
        self.net = nn.Sequential(
            ...
            # nn.LayerNorm(hidden_dim),  # 归一化，稳定数值分布
            # nn.GELU(),  # GELU 通常比 ReLU 在 Transformer 中表现更好

            # 第二层：投影到目标 Embedding 维度
...
class Actor_CrossAttentionMLP(Module):
    def __init__(self, weight_decay=1e-4):
        ...
        # ------------------------------------------------------
        # 1. 实体编码层 (使用增强的 EntityEncoder)
        # ------------------------------------------------------
        # 中间层维度设为 128，给予网络足够的宽度来组合特征
        encoder_hidden_dim = 64

        self.missile_encoder = EntityEncoder(
            input_dim=MISSILE_FEAT_DIM,
            output_dim=ENTITY_EMBED_DIM,
            hidden_dim=encoder_hidden_dim
        )
        self.aircraft_encoder = EntityEncoder(
            input_dim=AIRCRAFT_FEAT_DIM,
            output_dim=ENTITY_EMBED_DIM,
            hidden_dim=encoder_hidden_dim
        )
        ...
        # ------------------------------------------------------
        # 3. 混合 MLP 架构 (Shared Base + Towers)
        # ------------------------------------------------------
        # 输入由两部分拼接而成: [飞机自身的Embedding, Attention输出的威胁上下文]
        ...
        # self._init_output_weights()  # 仅初始化输出头，Encoder已在内部初始化

    def _init_output_weights(self):
        # 针对输出头进行特殊初始化，使初始策略接近随机（高熵）
    ...
    def forward(self, obs):
        ...
        # 2. 构建掩码 (处理无效/消失的导弹)
        # 指纹: [dist_norm=1.0, sin(beta)=0.0, cos(beta)=1.0, theta_norm=0.0]
        ...
        # Key Padding Mask: True 表示被掩盖(忽略)
        ...
        # Query: 我方飞机 (Batch, 1, Embed)
        aircraft_query = ac_embed.unsqueeze(1)
        # Key/Value: 导弹列表 (Batch, 2, Embed)
        missile_kv = torch.stack([m1_embed, m2_embed], dim=1)
        ...
        # 5. 特征融合
        # 去掉序列维度: (Batch, 1, Embed) -> (Batch, Embed)
        attn_output_squeezed = attn_output.squeeze(1)

        # [核心]: 拼接 "原始飞机状态" 和 "环境威胁上下文"
        # 这样 MLP 既拥有绝对飞行数据，也拥有相对威胁感知
        ...
        # 9. 整理注意力权重用于输出 (用于奖励函数)
        # (Batch, NumHeads, 1, 2) -> (Batch, 2)
...
class Critic_CrossAttentionMLP(Module):
    def __init__(self, weight_decay=1e-4):
        ...
        for i, dim in enumerate(mlp_dims):
            self.post_attention_mlp.add_module(f'mlp_fc_{i}', Linear(current_dim, dim))
            self.post_attention_mlp.add_module(f'mlp_leakyrelu_{i}', LeakyReLU())
            current_dim = dim

        self.fc_out = Linear(current_dim, self.output_dim)
...
#         mlp_input_dim = ENTITY_EMBED_DIM * 4 #* (1 + NUM_MISSILES)  # 64 * 4 = 256
#
#         split_point = 2
#         mlp_dims = ACTOR_PARA.model_layer_dim
#         base_dims = mlp_dims[:split_point]
...
#         combined_features = torch.cat([ac_embed, attn_output_squeezed, m1_embed, m2_embed], dim=-1)
#
...
#
#         # <<< 核心修改 1: 更新MLP的输入维度 >>>
#         mlp_input_dim = ENTITY_EMBED_DIM * 4   #* (1 + NUM_MISSILES)  # 192
#
