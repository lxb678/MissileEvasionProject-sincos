...
class Actor_CrossAttentionMLP(Module):
    ...
    def __init__(self, weight_decay=1e-4):
        ...
        self.missile_encoder_k = Sequential(
            Linear(MISSILE_FEAT_DIM, ENTITY_EMBED_DIM),
            # GELU()
            # Tanh()
            # LeakyReLU()  # <-- 添加非线性激活
        )
        self.missile_encoder_v = Sequential(
            Linear(MISSILE_FEAT_DIM, ENTITY_EMBED_DIM),
            # GELU()
            # Tanh()
            # LeakyReLU()  # <-- 添加非线性激活
    ...
    def forward(self, obs):
        ...
        m1_embed_k = self.missile_encoder_k(missile1_obs)
        m2_embed_k = self.missile_encoder_k(missile2_obs)
        m1_embed_v = self.missile_encoder_v(missile1_obs)
        m2_embed_v = self.missile_encoder_v(missile2_obs)
        ...
        missile_entities = torch.stack([m1_embed_k, m2_embed_k], dim=1)

        # Value 使用 Raw (关键修改！！！)
        # 这样注意力输出的特征就保留了原始的物理强度信息
        missile_values_raw = torch.stack([m1_embed_v, m2_embed_v], dim=1)
