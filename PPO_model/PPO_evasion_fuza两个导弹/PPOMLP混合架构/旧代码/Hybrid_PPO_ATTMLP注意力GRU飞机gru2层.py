# 文件名: Hybrid_PPO_CrossAttentionMLP_AirGRU_NoMissileGRU_NoAirEncoder.py
# 描述:
#      1. 导弹流：Encoder -> (无GRU) -> Attention -> MLP (保持上个版本的修改)
#      2. 飞机流：(无Encoder) -> GRU -> Attention -> MLP (本次修改：去掉飞机编码层，原始数据直入GRU)
...
# ==============================================================================
#           <<< 核心修改 >>>: 飞机移除Encoder，直接进GRU
# ==============================================================================

class Actor_CrossAttentionMLP(Module):
    """
    Actor 网络 - [架构:
        Missile: Encoder -> Attention
        Air:     Raw Input -> GRU -> Attention
    ]
    """
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        # 配置部分
        # =====================================================================
        self.rnn_hidden_dim = ENTITY_EMBED_DIM
        ...
        # [修改] 移除了 self.aircraft_encoder

        # =====================================================================
        # 2. GRU 层 (仅保留飞机)
        # =====================================================================
        # [修改] self.aircraft_gru 的 input_size 改为 FULL_OBS_DIM (直接吃原始数据)

        self.num_layers = 2  # 或者作为参数传入

        self.aircraft_gru = nn.GRU(
            input_size=FULL_OBS_DIM,
            hidden_size=self.rnn_hidden_dim,
            num_layers=self.num_layers,  # 设置层数
            batch_first=True
        )
        ...
        # 4. MLP 决策层 (保持不变)
        # ======================================================================
        mlp_input_dim = self.rnn_hidden_dim * 2
        split_point = 2
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # ======================================================================
        # <<< 修改 >>>: 强制隐状态连续，防止 RuntimeError: rnn: hx is not contiguous
        # ======================================================================
        if h_air is not None:
            h_air = h_air.contiguous()
        ...
        # ======================================================================
        # 导弹流: Encoder -> (无GRU) -> Attention (保持不变)
        # ======================================================================
        ...
        # ======================================================================
        # [修改] 飞机流: Raw Input -> GRU
        # ======================================================================
        # 1. 直接准备原始数据，不经过 aircraft_encoder
        air_raw_seq = obs_tensor.view(batch_size, seq_len, FULL_OBS_DIM)

        # 2. GRU
        gru_out_air, next_h_air = self.aircraft_gru(air_raw_seq, h_air)

        # 3. [修改] 移除残差连接 (air_feat_seq = gru_out_air + air_embed_seq)，因为维度对不上了且不再需要
        air_feat_seq = gru_out_air
...
class Critic_CrossAttentionMLP(Module):
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # 3. [修改] 移除残差连接
        air_feat_flat = gru_out_air.reshape(-1, self.rnn_hidden_dim)

        # ======================================================================
        # 导弹流: Encoder -> (无GRU) -> Attention
        # ======================================================================
        missiles_raw = torch.cat([missile1_obs, missile2_obs], dim=0)
        missiles_embed_flat = self.missile_encoder(missiles_raw.view(-1, MISSILE_FEAT_DIM))
        missiles_embed_seq = missiles_embed_flat.view(batch_size * 2, seq_len, self.rnn_hidden_dim)
...
class PPO_continuous(object):
    def __init__(self, load_able: bool, model_dir_path: str = None, use_rnn: bool = True):
        ...
        print(f"--- 初始化 PPO Agent (Cross-Attention + AirGRU_Only + NoAirEncoder) use_rnn={self.use_rnn} ---")
        ...
        self.training_start_time = time.strftime("PPO_OneGRU_NoEnc_%Y-%m-%d_%H-%M-%S")
    ...
    def choose_action(self, state, deterministic=False):
        ...
        with torch.no_grad():
            ...
            # ======================================================================
            # 初始化 temp_hidden: 维度仅为 1 个 hidden_dim (仅飞机)
            # ======================================================================
            if current_actor_h is None:
                batch_size = state_tensor.shape[0]
                total_hidden_dim = self.Actor.rnn_hidden_dim
                self.temp_actor_h = torch.zeros(self.Actor.num_layers, batch_size, total_hidden_dim).to(ACTOR_PARA.device)
                self.temp_critic_h = torch.zeros(self.Critic.num_layers, batch_size, total_hidden_dim).to(CRITIC_PARA.device)
            else:
