...
class Actor_CrossAttentionMLP(Module):
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        # 配置部分: 统一所有维度定义
        # =====================================================================
        self.rnn_hidden_dim = 128  # GRU 的隐藏层维度
        self.entity_embed_dim = 128  # 实体嵌入维度 (用于Attention)
        self.encoder_hidden_dim = 128  # 编码器的中间层维度

        # 确保维度兼容性
        assert self.rnn_hidden_dim == self.entity_embed_dim, \
            "为了残差连接,GRU输出和编码器输出必须维度相同"

        # =====================================================================
        # 1. GRU 层 (时序特征提取)
        # =====================================================================
        self.missile_gru = nn.GRU(
            input_size=MISSILE_FEAT_DIM,  # 4维原始观测
            hidden_size=self.rnn_hidden_dim,  # 64维隐藏状态
            batch_first=True
        )

        self.aircraft_gru = nn.GRU(
            input_size=FULL_OBS_DIM,  # 15维原始观测
        ...
        self.attention = MultiheadAttention(
            embed_dim=self.entity_embed_dim,  # ✅ 使用正确的变量名
