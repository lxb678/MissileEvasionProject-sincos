...
# ==============================================================================
#           <<< 核心修改 >>>: Critic 去掉 GRU，回归 Attention + MLP
# ==============================================================================

class Critic_PostAttentionGRU(Module):
    """
    Critic 网络 - [架构: Encoders -> Attention -> Fusion -> MLP]
    注意：虽然类名没改（为了兼容接口），但内部 GRU 已移除。
    """
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        # 配置 (rnn_hidden_dim 参数仅为了保持接口兼容，实际不用)
        self.entity_embed_dim = ENTITY_EMBED_DIM
        self.encoder_hidden_dim = ENTITY_EMBED_DIM
        ...
        # 3. MLP (去掉 GRU，直接接 MLP)
        mlp_dims = CRITIC_PARA.model_layer_dim
        self.mlp = Sequential()

        # 输入维度 = 飞机特征 + Attention特征
        input_dim = self.entity_embed_dim * 2
    ...
    def forward(self, obs, rnn_state=None):
        # 注意：rnn_state 参数被保留以兼容 PPO 的调用接口，但在内部被忽略
        obs_tensor = check(obs).to(**ACTOR_PARA.tpdv)
        if obs_tensor.dim() == 2:
            obs_tensor = obs_tensor.unsqueeze(1)
        batch_size, seq_len, _ = obs_tensor.shape
        ...
        # 3. Fusion
        # 直接拼接 [Aircraft, Attention_Context]
        fusion_features_flat = torch.cat([air_embed_flat, attn_out.squeeze(1)], dim=-1)

        # 4. MLP (直接计算 Value)
        val = self.fc_out(self.mlp(fusion_features_flat))

        # 为了兼容 PPO 接口，返回 None 或 原始 rnn_state 作为 "next_h"
        return val, rnn_state
...
class PPO_continuous(object):
    ...
    def choose_action(self, state, deterministic=False):
        ...
        with torch.no_grad():
            ...
            # <<< 修改点: Critic 调用不需要传有效 Hidden State，也不更新 self.critic_rnn_state >>>
            ...
            # ======================================================================
            # [修正后的代码] 初始化 temp_hidden
            # ======================================================================
            batch_size = state_tensor.shape[0]

            # 1. 处理 Actor Hidden (保持 GRU 逻辑)
            if current_actor_h is None:
                total_hidden_dim_a = self.Actor.rnn_hidden_dim
                self.temp_actor_h = torch.zeros(1, batch_size, total_hidden_dim_a).to(ACTOR_PARA.device)
            else:
                self.temp_actor_h = current_actor_h

            # 2. 处理 Critic Hidden
            # [!!! 关键修改 !!!]
            # Buffer 期望的是 (Layers, Hidden) 这种 2维形状，而不是 (Layers, Batch, Hidden) 的 3维形状。
            # 所以我们需要 .squeeze(1) 去掉中间的 Batch 维度。
            # 这样堆叠后就是 (Seq_Len, Layers, Hidden)，满足 transpose (1, 0, 2) 的需求。
            total_hidden_dim_c = 1
            self.temp_critic_h = torch.zeros(1, batch_size, total_hidden_dim_c).to(CRITIC_PARA.device).squeeze(1)
        ...
        for _ in range(self.ppo_epoch):
            ...
            for batch_data in batch_generator:
                if self.use_rnn:
                    ...
                    # <<< 修改点: Critic 计算 Value 时忽略 GRU 状态 >>>
                    # 我们传入 None 或者不传，因为我们在 Critic forward 里已经忽略了它
                    new_value, _ = self.Critic(state, None)
