# 文件名: Hybrid_PPO_CrossAttentionMLP_GRU_After_Attn_Strict.py
# 描述: 严格修改版 - 仅将GRU移至Attention之后，不添加任何额外激活函数。
#      位置：Encoder -> Attention -> GRU -> MLP。
...
# ==============================================================================
#           <<< 核心修改 >>>: GRU 移至 Attention 后，且不加激活函数
# ==============================================================================

class Actor_CrossAttentionMLP(Module):
    """
    Actor 网络 - [架构: Encoder -> Attention -> GRU -> MLP]
    """
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        # 3. GRU 层 (位置改变)
        # =====================================================================
        # 之前的 aircraft_gru 现在处理融合后的特征，改名为 global_gru
        # input_size 依然是 128 (因为 Attention 输出 + Residual 也是 128)
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # 1. 预处理 & 编码 (Encoder)
        # ======================================================================
        # 展平处理，把 Batch 和 Seq 合并，以便进行 Frame-wise 的 Attention
        obs_flat = obs_tensor.reshape(-1, FULL_OBS_DIM)

        missile1_raw = obs_flat[..., 0:MISSILE_FEAT_DIM]
        missile2_raw = obs_flat[..., MISSILE_FEAT_DIM:2 * MISSILE_FEAT_DIM]

        # 编码 (不经过 GRU，直接 Linear)
        air_embed_flat = self.aircraft_encoder(obs_flat)
        m1_embed_flat = self.missile_encoder(missile1_raw)
        m2_embed_flat = self.missile_encoder(missile2_raw)

        # ======================================================================
        # 2. Attention (Frame-wise)
        # ======================================================================
        # 准备 Mask
        inactive_fingerprint = torch.tensor([1.0, 0.0, 1.0, 0.0], device=obs_tensor.device)
        is_m1_inactive = torch.all(torch.isclose(missile1_raw, inactive_fingerprint), dim=-1)
        is_m2_inactive = torch.all(torch.isclose(missile2_raw, inactive_fingerprint), dim=-1)
        attention_mask = torch.stack([is_m1_inactive, is_m2_inactive], dim=1)

        # 准备 Query / Key / Value
        # 维度: (Batch*Seq, 1, Hidden) 和 (Batch*Seq, 2, Hidden)
        query = air_embed_flat.unsqueeze(1)
        keys = torch.stack([m1_embed_flat, m2_embed_flat], dim=1)
        ...
        # 残差连接 (飞机特征 + 注意力特征)
        combined_features_flat = air_embed_flat + attn_output.squeeze(1)

        # ======================================================================
        # 3. GRU (在 Attention 之后处理时序)
        # ======================================================================
        # 恢复序列维度: (Batch, Seq, Hidden)
        combined_features_seq = combined_features_flat.reshape(batch_size, seq_len, self.entity_embed_dim)

        # GRU 前向传播
        gru_out_seq, next_rnn_state = self.global_gru(combined_features_seq, rnn_state)

        # 再次展平给 MLP
        mlp_input_features = gru_out_seq.reshape(-1, self.rnn_hidden_dim)

        # ======================================================================
        # 4. MLP 决策
        # ======================================================================
        base_features = self.shared_base_mlp(mlp_input_features)
...
class Critic_CrossAttentionMLP(Module):
    """
    Critic 网络 - [架构: Encoder -> Attention -> GRU -> MLP]
    """
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        self.mlp = Sequential()
        input_dim = self.rnn_hidden_dim  # 来自 GRU
    ...
    def forward(self, obs, rnn_state=None):
        ...
        air_embed_flat = self.aircraft_encoder(obs_flat)
        m1_embed_flat = self.missile_encoder(m1_raw)
        m2_embed_flat = self.missile_encoder(m2_raw)
        ...
        combined_features_flat = air_embed_flat + attn_out.squeeze(1)
        ...
        # 4. MLP
        # ======================================================================
        val = self.fc_out(self.mlp(mlp_input_features))
...
class PPO_continuous(object):
    def __init__(self, load_able: bool, model_dir_path: str = None, use_rnn: bool = True):
        ...
        print(f"--- 初始化 PPO Agent (Encoder -> Attn -> GRU -> MLP) use_rnn={self.use_rnn} ---")
        ...
        self.training_start_time = time.strftime("PPO_AttnBeforeGRU_%Y-%m-%d_%H-%M-%S")
        ...
        for _ in range(self.ppo_epoch):
            ...
            for batch_data in batch_generator:
                if self.use_rnn:
                    ...
                    # 使用 Burn-in 后的 hidden state 进行训练
