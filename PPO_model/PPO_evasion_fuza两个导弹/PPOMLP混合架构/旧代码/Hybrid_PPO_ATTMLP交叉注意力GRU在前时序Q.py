...
class Actor_CrossAttentionMLP(Module):
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=64):
        ...
        # 导弹瞬时特征编码器 (替代 missile_gru)
        self.missile_encoder = nn.Sequential(
            nn.Linear(MISSILE_FEAT_DIM, self.rnn_hidden_dim),
            # nn.LeakyReLU()
        )
    ...
    def forward(self, obs, rnn_state=None):
        ...
        rnn_state: (1, Batch, Hidden) -> 现在只有一个 GRU 流
        """
        ...
        # --- 1. GRU 编码 (Temporal Encoding for Global Context) ---
        # 整个观测都送入 GRU，提取轨迹特征
        # output: (Batch, Seq, Hidden)
        context_out, next_rnn_state = self.context_gru(obs_tensor, rnn_state)

        # --- 2. 准备 Attention 输入 ---
        # 展平 (Batch, Seq, Hidden) 为 (Batch * Seq, Hidden)
        context_feat_flat = context_out.reshape(-1, self.rnn_hidden_dim)  # (B*S, H)

        # 展平原始观测以提取当前帧的导弹特征和掩码
        ...
        # --- 3. 导弹瞬时特征编码 ---
        m1_feat_encoded = self.missile_encoder(m1_obs_flat)  # (B*S, H)
        m2_feat_encoded = self.missile_encoder(m2_obs_flat)  # (B*S, H)
        ...
        # Query: 全局时序上下文特征
        # Key/Value: 导弹当前时刻的编码特征
        query = context_feat_flat.unsqueeze(1)  # (B*S, 1, H)
        keys = torch.stack([m1_feat_encoded, m2_feat_encoded], dim=1)  # (B*S, 2, H)
        values = keys
        ...
        # 残差连接：全局时序特征 + 对威胁的关注结果
        combined_features = context_feat_flat + attn_output.squeeze(1)
...
class Critic_CrossAttentionMLP(Module):
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=64):
        ...
        # 1. Encoders
        self.context_gru = nn.GRU(FULL_OBS_DIM, self.rnn_hidden_dim, batch_first=True)
        self.missile_encoder = nn.Sequential(
            nn.Linear(MISSILE_FEAT_DIM, self.rnn_hidden_dim),
            # nn.LeakyReLU()
        )
    ...
    def forward(self, obs, rnn_state=None):
        ...
        context_flat = context_out.reshape(-1, self.rnn_hidden_dim)

        # 提取并编码导弹瞬时特征
        obs_flat = obs_tensor.view(-1, FULL_OBS_DIM)
        m1_obs_flat = obs_flat[..., 0:MISSILE_FEAT_DIM]
        m2_obs_flat = obs_flat[..., MISSILE_FEAT_DIM:2 * MISSILE_FEAT_DIM]
        m1_encoded = self.missile_encoder(m1_obs_flat)
        m2_encoded = self.missile_encoder(m2_obs_flat)

        # Mask
        inactive = torch.tensor([1.0, 0.0, 1.0, 0.0], device=obs_tensor.device)
        mask = torch.stack([
            torch.all(torch.isclose(m1_obs_flat, inactive), dim=-1),
            torch.all(torch.isclose(m2_obs_flat, inactive), dim=-1)
        ], dim=1)

        # Attention
        query = context_flat.unsqueeze(1)
        keys = torch.stack([m1_encoded, m2_encoded], dim=1)
        ...
        # MLP
        combined = context_flat + attn_out.squeeze(1)
...
class PPO_continuous(object):
    ...
    def choose_action(self, state, deterministic=False):
        ...
        with torch.no_grad():
            ...
            # --- START MODIFICATION 3.1 ---
            # <<< 2. 保存旧状态到 self，供 store_experience 调用 >>>
            # 如果是 None (第一步)，创建一个全0的 hidden state
            # 注意：这里的维度现在只是 rnn_hidden_dim
            if current_actor_h is None:
                hidden_dim = self.Actor.rnn_hidden_dim
                self.temp_actor_h = torch.zeros(1, 1, hidden_dim).to(ACTOR_PARA.device)
                self.temp_critic_h = torch.zeros(1, 1, hidden_dim).to(CRITIC_PARA.device)
            else:
