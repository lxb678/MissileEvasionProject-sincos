...
class Actor_CrossAttentionMLP(Module):
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        # 配置部分
        # =====================================================================
        self.rnn_hidden_dim = ENTITY_EMBED_DIM#64 #128  # GRU 内部维度 (仅用于飞机)
        self.entity_embed_dim = ENTITY_EMBED_DIM#64 #128  # Attention 维度
        self.encoder_hidden_dim = ENTITY_EMBED_DIM#64 #128
        ...
        # self.dropout = nn.Dropout(p=0.3)  # 添加 Dropout 层，建议 0.2~0.5
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # gru_out_air = self.dropout(gru_out_air)  # 在输出前进行 Dropout
        ...
        combined_features = torch.cat([air_feat_flat, attn_output.squeeze(1)], dim=-1)  # <--- 修改此处
        # MLP 决策
...
class Critic_CrossAttentionMLP(Module):
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        self.rnn_hidden_dim = ENTITY_EMBED_DIM#64 #128
        self.entity_embed_dim = ENTITY_EMBED_DIM#64 #128
        self.encoder_hidden_dim = ENTITY_EMBED_DIM#64 #128
    ...
    def forward(self, obs, rnn_state=None):
        ...
        combined = torch.cat([air_feat_flat, attn_out.squeeze(1)], dim=-1)  # <--- 修改此处
