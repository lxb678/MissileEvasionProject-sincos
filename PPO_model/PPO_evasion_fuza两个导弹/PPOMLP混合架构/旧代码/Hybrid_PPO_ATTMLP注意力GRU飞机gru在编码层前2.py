# 文件名: Hybrid_PPO_CrossAttentionMLP_AirGRU_BeforeEncoder.py
# 描述: 修改版 - 飞机 GRU 移动到 编码层 之前。
#      位置：(仅飞机 GRU -> Encoder) / (导弹直接 Encoder) -> Attention -> MLP。
...
# ==============================================================================
#           <<< 核心修改 >>>: 飞机GRU前置到Encoder之前
# ==============================================================================

class Actor_CrossAttentionMLP(Module):
    """
    Actor 网络 - [架构: (Air: GRU -> Encoder / M: Encoder) -> Attention -> MLP]
    """
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        # 配置部分
        # =====================================================================
        self.rnn_hidden_dim = 64#128  # GRU 内部维度 (仅用于飞机)
        self.entity_embed_dim = 64##128  # Attention 维度
        self.encoder_hidden_dim =64# #128

        # =====================================================================
        # 1. GRU 层 (飞机) - [修改] 移到前面，输入维度为原始观测维度
        # =====================================================================
        self.aircraft_gru = nn.GRU(
            input_size=FULL_OBS_DIM,  # [修改] 输入为原始特征维度
        ...
        # [修改] 飞机编码器现在接在 GRU 后面，输入维度为 GRU 的隐藏层维度
        self.aircraft_encoder = Sequential(
            Linear(self.rnn_hidden_dim, self.encoder_hidden_dim),
        )

        # self.dropout = nn.Dropout(p=0.3)  # 添加 Dropout 层
        ...
        # 4. MLP 决策层 (保持不变)
        # ======================================================================
        mlp_input_dim = self.encoder_hidden_dim * 2
        split_point = 2
    ...
    def forward(self, obs, rnn_state=None):
        ...
        missiles_embed_seq = missiles_embed_flat.view(batch_size * 2, seq_len, self.encoder_hidden_dim)
        ...
        # 展平给 Attention 用
        m1_feat_flat = m1_feat_seq.reshape(-1, self.encoder_hidden_dim)
        m2_feat_flat = m2_feat_seq.reshape(-1, self.encoder_hidden_dim)

        # ======================================================================
        # 飞机流: GRU -> Encoder (位置互换)
        # ======================================================================
        # 1. 准备原始输入 (Batch, Seq, Feature)
        air_raw_flat = obs_tensor.reshape(-1, FULL_OBS_DIM)
        air_raw_seq = air_raw_flat.view(batch_size, seq_len, FULL_OBS_DIM)

        # 2. GRU (现在直接处理原始输入)
        gru_out_air, next_h_air = self.aircraft_gru(air_raw_seq, h_air)

        # 3. 编码器 (现在处理 GRU 的输出)
        gru_out_flat = gru_out_air.reshape(-1, self.rnn_hidden_dim)
        air_embed_flat = self.aircraft_encoder(gru_out_flat)
        # air_embed_seq = air_embed_flat.view(batch_size, seq_len, self.rnn_hidden_dim)

        # # 4. 残差连接 (Encoder输出 + GRU输出)
        # # 注: 原始代码是 Encoder+GRU，这里保持逻辑一致，即 线性层后+线性层前
        # air_feat_seq = air_embed_seq #+ gru_out_air
        # air_feat_flat = air_feat_seq.reshape(-1, self.rnn_hidden_dim)

        # [修改] 移除残差连接 (air_embed_seq + gru_out_air)，直接使用编码器输出
        air_feat_flat = air_embed_flat

        # ======================================================================
        # 打包 Next Hidden State
        # ======================================================================
        ...
        combined_features = torch.cat([air_feat_flat, attn_output.squeeze(1)], dim=-1)
        # MLP 决策
...
class Critic_CrossAttentionMLP(Module):
    """
    Critic 网络 - [架构: (Air: GRU -> Encoder / M: Encoder) -> Attention -> MLP]
    """
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        self.rnn_hidden_dim = 64#128
        self.entity_embed_dim = 64#128
        self.encoder_hidden_dim = 64#128
        ...
        # =====================================================================
        self.aircraft_gru = nn.GRU(
            input_size=FULL_OBS_DIM,  # [修改] 原始维度
            hidden_size=self.rnn_hidden_dim,
            batch_first=True
        )
        ...
        self.mlp = Sequential()
        input_dim = self.encoder_hidden_dim * 2
    ...
    def forward(self, obs, rnn_state=None):
        ...
        combined = torch.cat([air_feat_flat, attn_out.squeeze(1)], dim=-1)
...
class PPO_continuous(object):
    def __init__(self, load_able: bool, model_dir_path: str = None, use_rnn: bool = True):
        ...
        print(f"--- 初始化 PPO Agent (Cross-Attention + AirGRU_First) use_rnn={self.use_rnn} ---")
        ...
        self.training_start_time = time.strftime("PPO_AirGRUFirst_%Y-%m-%d_%H-%M-%S")
