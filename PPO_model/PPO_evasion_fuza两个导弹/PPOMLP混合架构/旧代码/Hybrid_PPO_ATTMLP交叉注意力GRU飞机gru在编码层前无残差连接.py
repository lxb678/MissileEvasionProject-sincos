...
class Actor_CrossAttentionMLP(Module):
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # GRU 输入: (Batch, Seq, Raw_Dim) -> 输出: (Batch, Seq, Hidden_Dim)
        gru_out, next_h_air = self.aircraft_gru(obs_tensor, h_air)

        next_rnn_state = next_h_air

        # 2. MLP 编码 (将 GRU 的时序特征投影到 Attention 空间)
        # 先 Flatten: (Batch*Seq, Hidden_Dim)
        gru_out_flat = gru_out.reshape(-1, self.rnn_hidden_dim)

        # 经过 Encoder
        air_embed_flat = self.aircraft_encoder(gru_out_flat)

        # [可选优化] 残差连接 (如果维度一致，推荐加上)
        # 因为 rnn_hidden_dim (64) == ENTITY_EMBED_DIM (64)
        # 这有助于保留原始的时序特征
        # air_feat_flat = air_embed_flat + gru_out_flat
        air_feat_flat = air_embed_flat
        # 如果不想加残差，直接用 air_feat_flat = air_embed_flat

        # --- 3. 准备 Attention ---
        # 此时 air_feat_flat 是 (Batch*Seq, 64)
...
class Critic_CrossAttentionMLP(Module):
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # --- 3. 准备 Attention ---
        air_feat_flat = air_feat_flat.reshape(-1, self.rnn_hidden_dim)
