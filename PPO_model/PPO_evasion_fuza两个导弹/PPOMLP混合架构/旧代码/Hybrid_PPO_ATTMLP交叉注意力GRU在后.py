...
class Actor_CrossAttentionMLP(Module):
    """
    Actor 网络 - [优化版：交叉注意力 + GRU + MLP]
    结构为: 实体编码器 -> 交叉注意力 -> 上下文融合 -> GRU(时序记忆) -> 共享MLP基座 -> 动作头。
    """
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=64):
        ...
        # <<< 核心修改: 加入 GRU 模块 >>>
        # 输入维度: Attention输出与飞机Embed融合后的维度 (此处为 ENTITY_EMBED_DIM)
        # 隐藏层维度: rnn_hidden_dim (默认 64)
        ...
        # --- 优化器设置 ---
        # ------------------------------------------------------------------
        # <<< 核心修改：参数分组，单独设置 GRU 学习率 >>>
        # ------------------------------------------------------------------
        attention_params = []
        gru_params = []  # 1. 新建 GRU 参数组
        other_params = []
        ...
        for name, param in self.named_parameters():
            ...
            elif 'gru' in name_lower:  # 2. 识别 GRU 参数 (因为 self.gru 命名包含 gru)
        ...
        param_groups = [
            ...
            {'params': gru_params, 'lr': ACTOR_PARA.gru_lr},  # 3. 设置 GRU 学习率
    ...
    def forward(self, obs, rnn_state=None):
        """
        obs: (Batch, Seq_Len, Dim) [训练时] 或 (Batch, Dim) [推理时]
        rnn_state: (1, Batch, Hidden)
        """
        ...
        ac_embed = self.aircraft_encoder(obs_flat)
        ...
        attn_output, attn_weights_raw = self.attention(
            query=aircraft_query,
            key=missile_entities,
            value=missile_values_raw,
        ...
        combined_features_flat = ac_embed + attn_output_squeezed

        # 3. 时序特征提取 (GRU)
        # 还原为 (Batch, Seq, Embed)
        gru_input = combined_features_flat.view(batch_size, seq_len, ENTITY_EMBED_DIM)

        # self.gru.flatten_parameters()
        gru_output, next_rnn_state = self.gru(gru_input, rnn_state)

        # 4. 决策 (MLP)
        # 再次展平 (B*S, Hidden)
        gru_output_flat = gru_output.reshape(batch_size * seq_len, self.rnn_hidden_dim)

        # <<< 核心修改: 使用残差连接替代拼接 >>>
        # combined_features_flat 是 "快车道" (瞬时信息)
        # gru_output_flat 是 "慢车道" (时序信息)
        mlp_input = combined_features_flat + gru_output_flat
        ...
        attention_to_missiles = attn_weights_raw.squeeze(1).view(batch_size, seq_len, 2)
        if seq_len == 1:
...
class Critic_CrossAttentionMLP(Module):
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=64):
        ...
        for name, param in self.named_parameters():
            ...
            if any(key in name_lower for key in ['attention', 'attn', 'layer_norm']):
                attention_params.append(param)
            elif 'gru' in name_lower:  # 2. 识别 GRU 参数
                gru_params.append(param)
            else:
                other_params.append(param)
        ...
        param_groups = [
            ...
            {'params': gru_params, 'lr': CRITIC_PARA.gru_lr},  # 3. 设置 GRU 学习率
    ...
    def forward(self, obs, rnn_state=None):
        ...
        aircraft_query = ac_embed.unsqueeze(1)
        missile_entities = torch.stack([m1_embed, m2_embed], dim=1)
        missile_values = torch.stack([m1_embed, m2_embed], dim=1)

        attn_output, _ = self.attention(
            query=aircraft_query,
            key=missile_entities,
            value=missile_values,
            key_padding_mask=attention_mask
        )
        ...
        value = self.fc_out(mlp_features)  # (B*S, 1)

        return value, next_rnn_state
    ...
    def choose_action(self, state, deterministic=False):
        ...
        with torch.no_grad():
            ...
            if current_actor_h is None:
                self.temp_actor_h = torch.zeros(1, 1, self.Actor.rnn_hidden_dim).to(ACTOR_PARA.device)
                self.temp_critic_h = torch.zeros(1, 1, self.Critic.rnn_hidden_dim).to(CRITIC_PARA.device)
            else:
