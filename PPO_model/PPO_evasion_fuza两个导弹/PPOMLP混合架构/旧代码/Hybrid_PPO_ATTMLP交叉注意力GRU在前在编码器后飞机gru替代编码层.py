...
class Actor_CrossAttentionMLP(Module):
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        # [修改] 移除 self.aircraft_encoder
        # self.aircraft_encoder = Sequential(...)  <-- 删除此部分

        # [修改] 2. 独立的 GRU (只给飞机用)
        # input_size 改为 FULL_OBS_DIM (直接吃原始数据)
        # hidden_size 保持 ENTITY_EMBED_DIM (64)
        self.aircraft_gru = nn.GRU(
            input_size=FULL_OBS_DIM,  # <--- 修改这里: 从 EMBED_DIM 改为 FULL_OBS_DIM
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # [修改] 移除飞机 MLP 编码
        # ac_embed_flat = self.aircraft_encoder(obs_flat) <-- 删除

        m1_embed_flat = self.missile_encoder(missile1_obs_flat)
        m2_embed_flat = self.missile_encoder(missile2_obs_flat)

        # ac_embed_seq = ac_embed_flat.view(batch_size, seq_len, ENTITY_EMBED_DIM) <-- 删除
        ...
        # [修改] 输入变成 obs_tensor (Batch, Seq, FULL_OBS_DIM)
        air_out, next_h_air = self.aircraft_gru(obs_tensor, h_air)

        # next_rnn_state 只需要返回飞机的 hidden state
        next_rnn_state = next_h_air

        # [修改] 移除残差连接
        # 因为输入是 Full Dim，输出是 64 Dim，维度不同无法相加
        # air_out = air_out + ac_embed_seq  <-- 删除此行

        # --- 3. 准备 Attention ---
        # 飞机特征来自 GRU 输出 (Batch, Seq, 64) -> Flatten
        air_feat_flat = air_out.reshape(-1, self.rnn_hidden_dim)

        # 导弹特征直接来自 Encoder (没有经过 GRU)
...
class Critic_CrossAttentionMLP(Module):
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        # [修改] 2. 独立的 GRU (只给飞机用)
        # input_size 改为 FULL_OBS_DIM
        self.aircraft_gru = nn.GRU(
            input_size=FULL_OBS_DIM,  # <--- 修改这里
            hidden_size=self.rnn_hidden_dim,
            batch_first=True
        )
