# 文件名: Hybrid_PPO_CrossAttentionMLP_AirGRU_MissileGRU.py
# 描述: 基于"编码层后接GRU"的版本修改。
#      位置：Encoder -> (飞机GRU / 导弹GRU) -> Attention -> MLP。
#      修改：给导弹加回了GRU模块，现在飞机和导弹是对称结构。
#      修复1：修复了 rnn_state 为 None 时的初始化。
#      修复2：修复了 'rnn: hx is not contiguous' 错误，增加了 .contiguous() 调用。
...
class Actor_CrossAttentionMLP(Module):
    """
    Actor 网络 - [架构: Encoder -> (Air:GRU / M:GRU) -> Attention -> MLP]
    """
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        # 配置部分
        # =====================================================================
        self.rnn_hidden_dim = 128  # GRU 内部维度
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # [修复] 必须处理 None 的情况，初始化全0状态
        if rnn_state is None:
            # 3 = 1个飞机 + 2个导弹
            rnn_state = torch.zeros(3, batch_size, self.rnn_hidden_dim, device=obs_tensor.device)

        # --- 1. Hidden State 处理 [修改] ---
        # rnn_state shape: [3, Batch, Hidden] -> [Air, M1, M2]

        # [关键修复] 调用 .contiguous() 解决 RuntimeError: rnn: hx is not contiguous
        h_air = rnn_state[0].unsqueeze(0).contiguous()  # [1, B, H]

        # 准备导弹状态
        h_m1 = rnn_state[1]  # [B, H]
        h_m2 = rnn_state[2]  # [B, H]
        # cat 之后通常是连续的，但加一个 contiguous 更保险
        h_missiles = torch.cat([h_m1, h_m2], dim=0).unsqueeze(0).contiguous()  # [1, B*2, H]

        # --- 数据提取 ---
        obs_flat = obs_tensor.reshape(-1, FULL_OBS_DIM)
        missile1_obs = obs_tensor[..., 0:MISSILE_FEAT_DIM]
        missile2_obs = obs_tensor[..., MISSILE_FEAT_DIM:2 * MISSILE_FEAT_DIM]

        # ======================================================================
        # 导弹流: Encoder -> GRU -> Attention
        # ======================================================================
        # 1. 拼接 (Batch*2, Seq, 4)
        ...
        # 3. GRU
        gru_out_missile, next_h_missiles = self.missile_gru(missiles_embed_seq, h_missiles)

        # 4. 残差连接
        missiles_feat_seq = gru_out_missile + missiles_embed_seq
        ...
        # ======================================================================
        # next_h_air: [1, B, H]
        # next_h_missiles: [1, B*2, H]

        # 将导弹状态拆回 M1 和 M2
        next_h_missiles_split = next_h_missiles.squeeze(0)  # [B*2, H]
        next_h_m1, next_h_m2 = torch.split(next_h_missiles_split, batch_size, dim=0)  # [B, H] each

        # 堆叠回 [3, B, H]
        next_rnn_state = torch.stack([next_h_air.squeeze(0), next_h_m1, next_h_m2], dim=0)

        # --- Attention 和 MLP 部分 (保持不变) ---

        # Mask 计算
        obs_flat_raw = obs_tensor.reshape(-1, FULL_OBS_DIM)
...
class Critic_CrossAttentionMLP(Module):
    """
    Critic 网络 - [架构: Encoder -> (Air:GRU / M:GRU) -> Attention -> MLP]
    """
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # --- 1. Hidden State 处理 [修改] ---
        # [关键修复] 调用 .contiguous()
        h_air = rnn_state[0].unsqueeze(0).contiguous()

        h_m1 = rnn_state[1]
        h_m2 = rnn_state[2]
        h_missiles = torch.cat([h_m1, h_m2], dim=0).unsqueeze(0).contiguous()
        ...
        # 飞机流: Encoder -> GRU (保持不变)
        # ======================================================================
        air_embed_flat = self.aircraft_encoder(obs_tensor.reshape(-1, FULL_OBS_DIM))
        air_embed_seq = air_embed_flat.view(batch_size, seq_len, self.rnn_hidden_dim)
        ...
        # [修改] 打包 Next Hidden State
        # ======================================================================
        next_h_missiles_split = next_h_missiles.squeeze(0)
        next_h_m1, next_h_m2 = torch.split(next_h_missiles_split, batch_size, dim=0)
        next_rnn_state = torch.stack([next_h_air.squeeze(0), next_h_m1, next_h_m2], dim=0)
...
class PPO_continuous(object):
    def __init__(self, load_able: bool, model_dir_path: str = None, use_rnn: bool = True):
        ...
        print(f"--- 初始化 PPO Agent (Cross-Attention + AirGRU + MissileGRU) use_rnn={self.use_rnn} ---")
        ...
        self.training_start_time = time.strftime("PPO_FullGRU_%Y-%m-%d_%H-%M-%S")
    ...
    def choose_action(self, state, deterministic=False):
        ...
        with torch.no_grad():
            ...
            # ======================================================================
            # [修改] 初始化 temp_hidden: 1 (Air) + 2 (Missiles) = 3
            # ======================================================================
            if current_actor_h is None:
                batch_size = state_tensor.shape[0]
                # [修改] 乘以 (1 + NUM_MISSILES) = 3
                num_entities = 1 + NUM_MISSILES
                hidden_dim = self.Actor.rnn_hidden_dim
                self.temp_actor_h = torch.zeros(num_entities, batch_size, hidden_dim).to(ACTOR_PARA.device)
                self.temp_critic_h = torch.zeros(num_entities, batch_size, hidden_dim).to(CRITIC_PARA.device)
            else:
    ...
    def learn(self, next_visual_value=0.0):
        ...
        # ======================================================================
        burn_in_len = 5
        train_len = self.rnn_seq_len
        total_seq_len = burn_in_len + train_len

        for _ in range(self.ppo_epoch):
            if self.use_rnn:
                batch_generator = self.buffer.generate_sequence_batches(
                    total_seq_len, self.rnn_batch_size, advantages, returns
                )
            else:
            ...
            for batch_data in batch_generator:
                if self.use_rnn:
                    ...
                    # Shape: [Batch, Total_Seq, Dim]
                    state_all = torch.FloatTensor(b_s).to(**ACTOR_PARA.tpdv)
                    action_all = torch.FloatTensor(b_a).to(**ACTOR_PARA.tpdv)
                    old_prob_all = torch.FloatTensor(b_p).to(**ACTOR_PARA.tpdv)
                    adv_all = torch.FloatTensor(b_adv).to(**ACTOR_PARA.tpdv).unsqueeze(-1)
                    ret_all = torch.FloatTensor(b_ret).to(**CRITIC_PARA.tpdv).unsqueeze(-1)

                    # 初始 Hidden State
                    # Shape: [Layers(3), Batch, Hidden]
                    ...
                    # Burn-in 阶段
                    # ==============================================================
                    burn_in_obs = state_all[:, :burn_in_len, :]

                    with torch.no_grad():
                        _, _, fresh_h_a = self.Actor(burn_in_obs, rnn_h_a)
                        _, fresh_h_c = self.Critic(burn_in_obs, rnn_h_c)

                    # ==============================================================
                    # Training 阶段
                    # ==============================================================
                    train_obs = state_all[:, burn_in_len:, :]

                    action_train = action_all[:, burn_in_len:, :]
                    action_batch = action_train.reshape(-1, action_train.shape[-1])

                    old_prob_train = old_prob_all[:, burn_in_len:]
                    old_prob = old_prob_train.reshape(-1)

                    adv_train = adv_all[:, burn_in_len:, :]
                    advantage = adv_train.reshape(-1, 1)

                    ret_train = ret_all[:, burn_in_len:, :]
                    return_ = ret_train.reshape(-1, 1)

                    new_dists, _, _ = self.Actor(train_obs, fresh_h_a)
                    new_value, _ = self.Critic(train_obs, fresh_h_c)
