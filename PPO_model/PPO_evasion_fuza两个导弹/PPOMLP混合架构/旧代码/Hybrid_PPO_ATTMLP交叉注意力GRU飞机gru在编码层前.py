# 文件名: Hybrid_PPO_CrossAttentionMLP_OnlyAirGRU.py
# 描述:
#      位置：Attention Fusion (空间特征) -> GRU (仅飞机时序) -> MLP (决策)。
#      修改：取消了导弹的GRU，导弹特征直接通过MLP编码进入Attention层。
#           飞机保留GRU以记忆自身轨迹和整体状态的历史信息。
...
class Actor_CrossAttentionMLP(Module):
    """
    Actor 网络 - [架构: 导弹MLP/飞机GRU -> Attention -> MLP]
    """
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        # 配置部分
        # =====================================================================
        self.rnn_hidden_dim = 128  # GRU 的隐藏层维度 (仅用于飞机)
        self.entity_embed_dim = 128  # 实体嵌入维度 (Attention维度)
        ...
        self.aircraft_gru = nn.GRU(
            input_size=FULL_OBS_DIM,  # 飞机流输入全量观测
            hidden_size=self.rnn_hidden_dim,
            batch_first=True
        )

        # [修改] 移除了 self.ln_gru_missile
        self.ln_gru_air = nn.LayerNorm(self.rnn_hidden_dim)

        # =====================================================================
        # 2. 编码器
        # =====================================================================
        # [修改] 导弹编码器直接接受原始特征 (4维 -> 128维)
        self.missile_encoder = Sequential(
            Linear(MISSILE_FEAT_DIM, self.encoder_hidden_dim),
            LeakyReLU()  # 增加非线性，因为没有GRU了
        )
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # ======================================================================
        # [修改] Hidden State 处理
        # rnn_state 现在只包含飞机的状态 (1, Batch, Hidden)
        # ======================================================================
        h_air = rnn_state  # 之前需要split，现在不需要了，只有飞机有状态
        ...
        # ======================================================================
        # [修改] 导弹流处理：直接 MLP 编码，无 GRU
        # ======================================================================
        # 1. 拼接 M1 和 M2: (Batch*2, Seq, 4)
        missiles_in = torch.cat([missile1_obs, missile2_obs], dim=0)

        # 2. 展平以通过 Linear 层: (Batch*2*Seq, 4)
        missiles_flat_in = missiles_in.reshape(-1, MISSILE_FEAT_DIM)

        # 3. 编码: (Batch*2*Seq, H)
        missiles_embed_flat = self.missile_encoder(missiles_flat_in)

        # 4. 还原序列结构
        missiles_feat_seq = missiles_embed_flat.view(batch_size * 2, seq_len, self.rnn_hidden_dim)

        # 5. 切分为 M1 和 M2
        m1_feat_seq, m2_feat_seq = torch.split(missiles_feat_seq, batch_size, dim=0)

        # 6. 再次展平供 Attention 使用 (Batch*Seq, H)
        ...
        combined_features = self.attn_post_norm(air_feat_flat + attn_output.squeeze(1))
...
class Critic_CrossAttentionMLP(Module):
    """
    Critic 网络 - [架构: 导弹MLP/飞机GRU -> Attention -> MLP]
    """
    ...
    def forward(self, obs, rnn_state=None):
        ...
        combined = self.attn_post_norm(air_feat_flat + attn_out.squeeze(1))
...
class PPO_continuous(object):
    def __init__(self, load_able: bool, model_dir_path: str = None, use_rnn: bool = True):
        ...
        print(f"--- 初始化 PPO Agent (Cross-Attention + AircraftGRU + MissileMLP) use_rnn={self.use_rnn} ---")
