...
class Actor_CrossAttentionMLP(Module):
    """
    Actor 网络 - [架构重构：GRU (编码器) -> 交叉注意力 -> MLP]
    逻辑：先提取每个实体的时序轨迹特征，再通过注意力机制融合空间关系。
    """

    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=64):
        super(Actor_CrossAttentionMLP, self).__init__()
        self.input_dim = ACTOR_PARA.input_dim
        self.log_std_min = -20.0
        self.log_std_max = 2.0
        self.weight_decay = weight_decay
        self.rnn_hidden_dim = rnn_hidden_dim  # 单个实体的 Hidden Dim

        # ======================================================================
        # 1. GRU 编码器 (替代原本的 Linear Encoder)
        #    输入: 原始特征 -> 输出: 时序嵌入 (Temporal Embedding)
        # ======================================================================

        # 飞机专用 GRU
        self.aircraft_gru = nn.GRU(
            input_size=FULL_OBS_DIM,  # 飞机的观测通常包含自身状态
            hidden_size=self.rnn_hidden_dim,
            batch_first=True
        )

        # 导弹专用 GRU (参数共享，处理所有导弹)
        ...
        # ======================================================================
        # 2. 交叉注意力 (输入维度变为 GRU 的 hidden_dim)
        # ======================================================================
        self.attention = MultiheadAttention(embed_dim=self.rnn_hidden_dim,
                                            num_heads=ATTN_NUM_HEADS,
                                            dropout=0.0,
                                            batch_first=True)

        # ======================================================================
        # 3. MLP 决策层 (输入为 Attention 输出)
        # ======================================================================
    ...
    def forward(self, obs, rnn_state=None):
        ...
        rnn_state: (1, Batch, Hidden * 3) -> 因为我们有 3 个独立的流 (Air, M1, M2)
        """
        ...
        # --- 1. 数据拆分 ---
        # 展平以便处理，这里先保持 (B, S, Dim) 用于 GRU
        missile1_obs = obs_tensor[..., 0:MISSILE_FEAT_DIM]
        missile2_obs = obs_tensor[..., MISSILE_FEAT_DIM:2 * MISSILE_FEAT_DIM]
        # 飞机观测通常使用全量数据或特定部分，这里假设用全量全特征作为上下文
        aircraft_obs = obs_tensor

        # --- 2. 隐藏状态管理 (Splitting) ---
        # 我们需要把大的 hidden state 拆开给不同的 GRU 使用
        # rnn_state shape: (1, Batch, Hidden * 3)
        if rnn_state is None:
            ...
            # 切分 Hidden State
            # chunk 0: Air, chunk 1: M1, chunk 2: M2
            states = torch.split(rnn_state, self.rnn_hidden_dim, dim=-1)
            h_air = states[0].contiguous()
            h_m1 = states[1].contiguous()
            h_m2 = states[2].contiguous()

        # --- 3. GRU 编码 (Temporal Encoding) ---
        # 分别通过 GRU，提取轨迹特征
        # output: (Batch, Seq, Hidden)
        air_out, next_h_air = self.aircraft_gru(aircraft_obs, h_air)
        m1_out, next_h_m1 = self.missile_gru(missile1_obs, h_m1)
        m2_out, next_h_m2 = self.missile_gru(missile2_obs, h_m2)

        # 拼接新的 hidden state 返回 (1, Batch, Hidden * 3)
        next_rnn_state = torch.cat([next_h_air, next_h_m1, next_h_m2], dim=-1)

        # --- 4. 准备 Attention 输入 ---
        # 此时有了每个时间步的时序特征。
        # 我们需要将 (Batch, Seq, Hidden) 展平为 (Batch * Seq, Hidden) 来做 Attention
        # 因为 Attention 是处理每一帧的空间关系，而每一帧的特征现在包含了历史信息。

        air_feat_flat = air_out.reshape(-1, self.rnn_hidden_dim)  # (B*S, H)
        m1_feat_flat = m1_out.reshape(-1, self.rnn_hidden_dim)  # (B*S, H)
        m2_feat_flat = m2_out.reshape(-1, self.rnn_hidden_dim)  # (B*S, H)

        # 重新构建 Mask (需要原始数据)
        obs_flat = obs_tensor.view(-1, FULL_OBS_DIM)
        m1_obs_flat = obs_flat[..., 0:MISSILE_FEAT_DIM]
        m2_obs_flat = obs_flat[..., MISSILE_FEAT_DIM:2 * MISSILE_FEAT_DIM]

        inactive_fingerprint = torch.tensor([1.0, 0.0, 1.0, 0.0], device=obs_tensor.device)
        is_m1_inactive = torch.all(torch.isclose(m1_obs_flat, inactive_fingerprint), dim=-1)
        is_m2_inactive = torch.all(torch.isclose(m2_obs_flat, inactive_fingerprint), dim=-1)
        attention_mask = torch.stack([is_m1_inactive, is_m2_inactive], dim=1)  # (B*S, 2)

        # --- 5. 交叉注意力 (Spatial Fusion) ---
        # Query: 飞机轨迹特征
        # Key/Value: 导弹轨迹特征
        query = air_feat_flat.unsqueeze(1)  # (B*S, 1, H)
        keys = torch.stack([m1_feat_flat, m2_feat_flat], dim=1)  # (B*S, 2, H)
        values = keys
...
class Critic_CrossAttentionMLP(Module):
    """
    Critic 网络 - [架构重构：GRU (编码器) -> 交叉注意力 -> MLP]
    """
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=64):
        ...
        # 1. GRU Encoders
        self.aircraft_gru = nn.GRU(FULL_OBS_DIM, self.rnn_hidden_dim, batch_first=True)
        self.missile_gru = nn.GRU(MISSILE_FEAT_DIM, self.rnn_hidden_dim, batch_first=True)

        # 2. Attention
        self.attention = MultiheadAttention(embed_dim=self.rnn_hidden_dim, num_heads=ATTN_NUM_HEADS, batch_first=True)
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # 拆分数据
        m1_obs = obs_tensor[..., 0:MISSILE_FEAT_DIM]
        m2_obs = obs_tensor[..., MISSILE_FEAT_DIM:2 * MISSILE_FEAT_DIM]
        air_obs = obs_tensor
        ...
        if rnn_state is None:
            ...
            states = torch.split(rnn_state, self.rnn_hidden_dim, dim=-1)
            h_air, h_m1, h_m2 = states[0].contiguous(), states[1].contiguous(), states[2].contiguous()

        # GRU 编码
        air_out, next_h_air = self.aircraft_gru(air_obs, h_air)
        m1_out, next_h_m1 = self.missile_gru(m1_obs, h_m1)
        m2_out, next_h_m2 = self.missile_gru(m2_obs, h_m2)
        next_rnn_state = torch.cat([next_h_air, next_h_m1, next_h_m2], dim=-1)

        # 准备 Attention 输入 (Flatten B and S)
        air_flat = air_out.reshape(-1, self.rnn_hidden_dim)
        m1_flat = m1_out.reshape(-1, self.rnn_hidden_dim)
        m2_flat = m2_out.reshape(-1, self.rnn_hidden_dim)

        # Mask
        obs_flat = obs_tensor.view(-1, FULL_OBS_DIM)
        m1_raw = obs_flat[..., 0:MISSILE_FEAT_DIM]
        m2_raw = obs_flat[..., MISSILE_FEAT_DIM:2 * MISSILE_FEAT_DIM]
        ...
        # Attention
        query = air_flat.unsqueeze(1)
        keys = torch.stack([m1_flat, m2_flat], dim=1)
        ...
        # MLP
        combined = air_flat + attn_out.squeeze(1)
...
class PPO_continuous(object):
    ...
    def choose_action(self, state, deterministic=False):
        ...
        with torch.no_grad():
            ...
            # 注意：这里的维度变成了 Hidden * 3
            if current_actor_h is None:
                # 3倍宽度，因为拼接了 Air, M1, M2
