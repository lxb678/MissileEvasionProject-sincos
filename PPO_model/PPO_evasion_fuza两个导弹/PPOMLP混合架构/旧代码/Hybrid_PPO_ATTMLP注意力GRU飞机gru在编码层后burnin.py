...
class Actor_CrossAttentionMLP(Module):
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        self.log_std_min = -2.0  # 限制标准差最小约为 0.13 不要让它变得太小   -20.0
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # --- 数据提取 ---
        # [修改] view -> reshape (解决 Burn-in 切片导致的内存不连续问题)
...
class PPO_continuous(object):
    ...
    def choose_action(self, state, deterministic=False):
        ...
        with torch.no_grad():
            ...
            # ======================================================================
            # [修改] 初始化 temp_hidden: 维度仅为 1 个 hidden_dim (仅飞机)
            # ======================================================================
    ...
    def learn(self, next_visual_value=0.0):
        ...
        # [修改 1] 定义 Burn-in 长度
        # ======================================================================
        burn_in_len = 5  # 预热步数，通常 5-10 步即可
        train_len = self.rnn_seq_len  # 实际用于训练的步数 (例如 10)
        total_seq_len = burn_in_len + train_len  # 总共需要从 Buffer 采样的长度 (15)

        for _ in range(self.ppo_epoch):
            if self.use_rnn:
                # ==================================================================
                # [修改 2] 请求更长的序列 (Burn-in + Train)
                # ==================================================================
            ...
            for batch_data in batch_generator:
                if self.use_rnn:
                    ...
                    # --- 将所有数据转为 Tensor，暂时保留 [Batch, Seq, Dim] 维度，不要 view(-1) ---
                    ...
                    # ==============================================================
                    # [修改 3] Burn-in 阶段 (关键步骤)
                    # ==============================================================
                    # 1. 切出前 burn_in_len 步的数据
                    burn_in_obs = state_all[:, :burn_in_len, :]

                    # 2. 用当前网络跑一遍 Burn-in 数据，不计算梯度
                    #    目的是让旧的 hidden state 演化成适应当前网络的新 hidden state
                    with torch.no_grad():
                        _, _, fresh_h_a = self.Actor(burn_in_obs, rnn_h_a)
                        _, fresh_h_c = self.Critic(burn_in_obs, rnn_h_c)

                    # fresh_h_a 就是我们要用于训练的“新鲜”初始状态

                    # ==============================================================
                    # [修改 4] Training 阶段 (使用新鲜状态)
                    # ==============================================================
                    # 1. 切出后 train_len 步的数据用于训练
                    train_obs = state_all[:, burn_in_len:, :]

                    # 2. 对应的 Target 数据也要切出来，并展平以计算 Loss
                    #    展平操作：[Batch, Seq, ...] -> [Batch*Seq, ...]
                    action_train = action_all[:, burn_in_len:, :]
                    action_batch = action_train.reshape(-1, action_train.shape[-1])  # 覆盖原来的变量名

                    old_prob_train = old_prob_all[:, burn_in_len:]
                    old_prob = old_prob_train.reshape(-1)  # 覆盖原来的变量名

                    adv_train = adv_all[:, burn_in_len:, :]
                    advantage = adv_train.reshape(-1, 1)  # 覆盖原来的变量名

                    ret_train = ret_all[:, burn_in_len:, :]
                    return_ = ret_train.reshape(-1, 1)  # 覆盖原来的变量名

                    # 3. 前向传播：使用 fresh_h_a 作为初始状态
                    #    注意：这里输入的是 train_obs
                    new_dists, _, _ = self.Actor(train_obs, fresh_h_a)
                    new_value, _ = self.Critic(train_obs, fresh_h_c)

                    # state 变量用于兼容后续代码 (虽然这里没再用到了)
                    state = train_obs
