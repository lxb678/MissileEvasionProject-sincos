...
class Actor_CrossAttentionMLP(Module):
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=64):
        ...
        self.aircraft_gru = nn.GRU(
            ...
            hidden_size=FULL_OBS_DIM,  # <-- 核心修改: hidden_size = input_size
            batch_first=True
        )

        self.missile_gru = nn.GRU(
            input_size=MISSILE_FEAT_DIM,
            hidden_size=MISSILE_FEAT_DIM,  # <-- 核心修改: hidden_size = input_size
            batch_first=True
        )

        # ======================================================================
        # 2. 实体编码器 (MLP)
        #    输入维度是GRU的输出维度，输出维度是用于注意力的 embed_dim
        # ======================================================================

        self.missile_encoder = Sequential(
            Linear(MISSILE_FEAT_DIM, ENTITY_EMBED_DIM),
            # LeakyReLU() # 根据你的需要，可以选择性开启
        )
        ...
        # ======================================================================
        # 3. 交叉注意力
        #    输入维度现在是 ENTITY_EMBED_DIM
        # ======================================================================
        self.attention = MultiheadAttention(embed_dim=ENTITY_EMBED_DIM,  # <-- 核心修改
        ...
        # ======================================================================
        # 4. MLP 决策层
        #    输入维度是 ENTITY_EMBED_DIM
        # ======================================================================
        mlp_input_dim = ENTITY_EMBED_DIM  # <-- 核心修改
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # --- 2. 隐藏状态管理 (Splitting with new dimensions) ---
        if rnn_state is None:
            h_air, h_m1, h_m2 = None, None, None
        else:
            # 切分 Hidden State
            # 注意：切分点现在是 FULL_OBS_DIM 和 MISSILE_FEAT_DIM
            h_air, h_m1, h_m2 = torch.split(
                rnn_state,
                [FULL_OBS_DIM, MISSILE_FEAT_DIM, MISSILE_FEAT_DIM],
                dim=-1
            )
        ...
        # <<< 新增：GRU后的残差连接 >>>
        air_out = air_out + aircraft_obs
        m1_out = m1_out + missile1_obs
        m2_out = m2_out + missile2_obs

        # 拼接新的 hidden state 返回
        next_rnn_state = torch.cat([next_h_air, next_h_m1, next_h_m2], dim=-1)

        # --- 4. 展平并送入 MLP 编码器 ---
        air_feat_flat = air_out.reshape(-1, FULL_OBS_DIM)
        m1_feat_flat = m1_out.reshape(-1, MISSILE_FEAT_DIM)
        m2_feat_flat = m2_out.reshape(-1, MISSILE_FEAT_DIM)

        ac_embed = self.aircraft_encoder(air_feat_flat)
        m1_embed = self.missile_encoder(m1_feat_flat)
        m2_embed = self.missile_encoder(m2_feat_flat)

        # --- 5. 准备 Attention 输入 ---
        # Mask (与之前相同)
        obs_flat = obs_tensor.view(-1, FULL_OBS_DIM)
        m1_obs_flat_raw = obs_flat[..., 0:MISSILE_FEAT_DIM]
        m2_obs_flat_raw = obs_flat[..., MISSILE_FEAT_DIM:2 * MISSILE_FEAT_DIM]
        inactive_fingerprint = torch.tensor([1.0, 0.0, 1.0, 0.0], device=obs_tensor.device)
        is_m1_inactive = torch.all(torch.isclose(m1_obs_flat_raw, inactive_fingerprint), dim=-1)
        is_m2_inactive = torch.all(torch.isclose(m2_obs_flat_raw, inactive_fingerprint), dim=-1)
        ...
        combined_features = ac_embed + attn_output.squeeze(1)
...
class Critic_CrossAttentionMLP(Module):
    """
    Critic 网络 - [架构重构：并行GRU -> 并行Encoder -> 注意力 -> MLP]
    """

    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=None):  # rnn_hidden_dim 不再使用
        ...
        self.aircraft_gru = nn.GRU(
            input_size=FULL_OBS_DIM,
            hidden_size=FULL_OBS_DIM,  # <-- 核心修改
            batch_first=True
        )
        self.missile_gru = nn.GRU(
            input_size=MISSILE_FEAT_DIM,
            hidden_size=MISSILE_FEAT_DIM,  # <-- 核心修改
            batch_first=True
        )
        ...
        self.attention = MultiheadAttention(
            embed_dim=ENTITY_EMBED_DIM,  # <-- 核心修改
            num_heads=ATTN_NUM_HEADS,
            batch_first=True
        )
        ...
        self.mlp = Sequential()
        input_dim = ENTITY_EMBED_DIM  # <-- 核心修改
    ...
    def forward(self, obs, rnn_state=None):
        ...
        if rnn_state is None:
            ...
            h_air, h_m1, h_m2 = torch.split(
                rnn_state,
                [FULL_OBS_DIM, MISSILE_FEAT_DIM, MISSILE_FEAT_DIM],  # <-- 新的切分点
                dim=-1
            )
            h_air, h_m1, h_m2 = h_air.contiguous(), h_m1.contiguous(), h_m2.contiguous()
        ...
        # --- 7. MLP 估值 ---
        combined = ac_embed + attn_out.squeeze(1)
    ...
    def choose_action(self, state, deterministic=False):
        ...
        with torch.no_grad():
            ...
            if current_actor_h is None:
                # 计算新的总隐藏维度
                total_hidden_dim = FULL_OBS_DIM + 2 * MISSILE_FEAT_DIM
