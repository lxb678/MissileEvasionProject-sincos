...
class Actor_CrossAttentionMLP(Module):
    """
    Actor 网络 - [架构优化: Encoder(特征提取) -> GRU(时序记忆) -> Attention -> MLP]
    """
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        # 确保维度兼容
        assert self.rnn_hidden_dim == self.entity_embed_dim
        ...
        # =====================================================================
        # 2. GRU 层 (在特征空间上进行时序推断)
        #    输入: 128维特征 -> 输出: 128维时序特征
        # =====================================================================
        self.missile_gru = nn.GRU(
            input_size=self.encoder_hidden_dim,  # 修改点: 接收编码后的特征 (128)
            hidden_size=self.rnn_hidden_dim,  # 128
            batch_first=True
        )
        ...
        # # GRU 输出归一化
        # self.ln_gru_missile = nn.LayerNorm(self.rnn_hidden_dim)
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # 3. GRU 时序记忆
        # Input: (B*2, S, 128), Hidden: (1, B*2, 128)
        gru_out_missiles, next_h_missiles = self.missile_gru(missiles_embed_seq, h_missiles)
        # gru_out_missiles = self.ln_gru_missile(gru_out_missiles)  # Norm

        # 4. 残差连接 (特征 + 时序)
        missiles_feat_seq = gru_out_missiles + missiles_embed_seq

        # 5. 还原 M1 和 M2
        # (Batch*2, Seq, 128) -> 切分 -> (Batch, Seq, 128)
...
class Critic_CrossAttentionMLP(Module):
    """
    Critic 网络 - [架构优化: Encoder -> GRU -> Attention -> MLP]
    """
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # 2. GRU
        gru_out_missiles, next_h_missiles = self.missile_gru(missiles_embed_seq, h_missiles)
        # gru_out_missiles = self.ln_gru_missile(gru_out_missiles)
