...
class Actor_PostAttentionGRU(Module):
    ...
    def __init__(self, weight_decay=1e-4, rnn_hidden_dim=ENTITY_EMBED_DIM):
        ...
        # =========================
        # 5) GRU 挪到 shared_base_mlp 后面（关键修改）
        # =========================
        self.global_gru = nn.GRU(
            input_size=base_output_dim,  # ✅ 改成共享层输出维度
            hidden_size=self.rnn_hidden_dim,
            batch_first=True
        )

        # =========================
        # 6) Tower 的输入维度改成：base_output_dim + rnn_hidden_dim（如果你要 skip-concat）
        # =========================
    ...
    def forward(self, obs, rnn_state=None):
        ...
        # 先喂共享层（注意：shared_base_mlp 是 Linear/激活，支持 3D 输入）
...
class PPO_continuous(object):
    def __init__(self, load_able: bool, model_dir_path: str = None, use_rnn: bool = True):
        ...
        self.rnn_seq_len = 5 #5 #15 #12 #15 #12 #20
    ...
    def choose_action(self, state, deterministic=False):
        ...
        with torch.no_grad():
            ...
            # --- 显式提取动作 (防止字典乱序) ---
            # 注意：Actor 返回的 key 是 'trigger' (见 Actor forward 最后几行)
    ...
    def learn(self, next_visual_value=0.0):
        ...
        for _ in range(self.ppo_epoch):
            ...
            for batch_data in batch_generator:
                ...
                # --- C. 离散动作 Log Prob (Ratio 一致性修复) ---
                # 1. 提取 Buffer 中的真实开火情况 (必须用 Buffer 里的数据！)
                actual_triggers = discrete_actions_from_buffer['trigger']

                # 2. Trigger LogProb (永远计算)
                # 注意：Actor 返回的 key 是 'trigger'
                ...
                # ) / num_sub_actions
                #
                # # 3. 🌟 灵魂掩码 (Entropy) 🌟：条件均值熵
                # # 注意：这里的数据可能已经被展平为 (B*S)，直接求和即可
                # num_triggered = actual_triggers.sum()
                # if num_triggered > 0:
                #     # 只计算那些【实际开火】样本的子动作熵
                #     # 重点：除以 num_triggered 而不是 batch_size
                ...
                # ================= [修改结束] =================
                    ...
                    # 4. 计算最终的 Loss (使用新的熵组合)
                # 连续动作平均熵 + Trigger平均熵 + 子动作条件平均熵
