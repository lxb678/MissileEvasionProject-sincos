...
from Interference_code.PPO_model.PPO_evasion.Buffer import *
...
class PPO_continuous(object):
   ...
   """
    ...
    def __init__(self, load_able: bool, model_dir_path: str = None):
        ...
        # 定义基础保存目录
        self.base_save_dir = "../../save/save_evade"
    ...
    # --- <<< 核心修改 3: 重命名旧的加载函数，使其职责更清晰 >>> ---
    def load_model_from_default_folder(self, save_dir="test"):
        """
        (旧逻辑) 从一个固定的默认文件夹 (如 "test") 自动搜索并加载模型。
        """
        # ... (您原来的 load_model_from_save_folder 函数的全部内容，复制到这里) ...
        # ... (为了简洁，这里省略，但您需要把它的代码完整地粘贴过来) ...
        ...
        if len(actor_files) == 1:
            ...
            for net in ['Actor', 'Critic']:
                try:
                    ...
                    getattr(self, net).load_state_dict(torch.load(filename, map_location=getattr(self, net).device))
            ...
            print("模型加载错误：在默认文件夹中未找到符合要求的模型文件。")
            return
