import torch
from torch.nn import *  # 导入 PyTorch 神经网络模块，如 Linear, Sequential, GRU, MultiheadAttention 等
from torch.distributions import Bernoulli, Categorical  # 导入用于离散动作的概率分布
from torch.distributions import Normal  # 导入用于连续动作的概率分布
# 导入配置文件，其中包含各种超参数
...
from torch.optim import lr_scheduler  # 导入学习率调度器，用于动态调整学习率
import numpy as np
import os
import re
import time
...
# --- 动作空间配置 (与原版相同) ---
# 定义连续动作的维度和键名
CONTINUOUS_DIM = 4  # 连续动作空间有4个维度
CONTINUOUS_ACTION_KEYS = ['throttle', 'elevator', 'aileron', 'rudder']  # 对应飞机的油门、升降舵、副翼、方向舵
# 定义离散动作的维度。每个键代表一个离散决策，值代表该决策有多少个选项。
DISCRETE_DIMS = {
    'flare_trigger': 1,  # 干扰弹触发，使用伯努利分布 (是/否)，所以是1个 logit
    'salvo_size': 3,  # 齐射数量，有3个选项
    'intra_interval': 3,  # 组内间隔，有3个选项
    'num_groups': 3,  # 组数，有3个选项
    'inter_interval': 3,  # 组间间隔，有3个选项
}
...
ATTN_NUM_HEADS = 2  # 1 #3 #4 #8 #4 #1 #2       # <<< 这是您的关键修改, 决定了多头注意力的"头"数
...
class Actor(Module):
    ...
    def __init__(self):
        ...
        self.input_dim = ACTOR_PARA.input_dim  # 从配置中读取状态输入的维度
        # 输出维度不再是一个单一的值，而是由连续和离散动作头决定
    ...
    def forward(self, obs):
        ...
        # --- 动作屏蔽 (Action Masking) ---
        # 获取状态中关于干扰弹数量的信息（假设在状态向量的索引7位置）
        has_flares_info = obs_tensor[:, 7]
        # 创建一个掩码，当干扰弹数量为0时为 True
        mask = (has_flares_info == 0)
        trigger_logits_masked = trigger_logits.clone()  # 复制一份 logits 以免修改原始输出
...
class Critic(Module):
    ...
    def __init__(self):
        ...
        self.input_dim = CRITIC_PARA.input_dim  # 状态输入维度
        self.output_dim = CRITIC_PARA.output_dim  # 输出维度，通常为1（一个标量价值）
        self.init_model()  # 初始化网络结构
        self.optim = torch.optim.Adam(self.parameters(), CRITIC_PARA.lr)  # 定义优化器
        # 定义学习率调度器
        ...
        self.to(CRITIC_PARA.device)  # 移动到指定设备
    ...
    def forward(self, obs):
        """前向传播，计算状态价值。"""
        obs = check(obs).to(**CRITIC_PARA.tpdv)  # 预处理输入
        value = self.network(obs)  # 通过网络计算价值
        return value
...
def init_weights(m, gain=1.0):
    """
    一个通用的权重初始化函数，用于改善训练的稳定性和收敛速度。
    :param m: PyTorch module (网络层)
    :param gain: 正交初始化的增益因子
    """
    if isinstance(m, Linear):
        # 对线性层使用 Kaiming Normal 初始化，它特别适用于带有 LeakyReLU 激活函数的网络
        ...
        # 对 GRU 的权重使用正交初始化，这有助于防止梯度爆炸/消失问题
...
# ==============================================================================
# <<< 新版本 >>>: 定义基于 [GRU -> Attention -> MLP] 的 Actor 和 Critic
# ==============================================================================

class Actor_GRU(Module):
    """
    Actor 网络 (策略网络) - 采用 [GRU -> 特征级 Attention -> MLP -> 动作头] 架构。

    该架构的核心思想是两阶段处理：
    1. 时序建模阶段（时间维度）：首先使用 GRU 处理整个输入序列 (Batch, Sequence, Features)，
       在每个时间步 `t` 生成一个包含了历史信息的隐藏状态向量 `h_t`。
    2. 特征交互阶段（特征维度）：在每个独立的时间步 `t`，将 GRU 输出的隐藏状态向量 `h_t`
       中的 G 个特征维度视为一个序列，使用自注意力机制计算这些特征之间的动态关系，
       从而生成一个对当前时刻上下文感知更强的特征表示。
    """
    ...
    def __init__(self, dropout_rate=0.05, weight_decay=1e-4):
        ...
        # self.rnn_hidden_size = RNN_HIDDEN_SIZE  # G
        self.rnn_hidden_size = self.input_dim  # 一个设计选择：让GRU隐藏层大小与输入维度相同
        self.attn_embedding_dim = 64  # 64  # n1, 特征嵌入后的维度
        self.weight_decay = weight_decay

        # --- 模块定义 ---

        # 1. GRU 时序建模层
        # 作用：处理原始输入序列 (B, S, D)，捕捉时间依赖性。输出形状为 (B, S, G)。
        self.gru = GRU(self.input_dim, self.rnn_hidden_size, batch_first=True)

        # 2. 特征嵌入层 (为 Attention 准备)
        # 作用：将 GRU 输出的 G 个特征中的每一个都视为一个 Token，并将其从维度 1 投影到高维嵌入空间 (attn_embedding_dim)。
        # 这是一个关键步骤，为特征间的注意力计算做准备。
        self.feature_embed_attn = Linear(1, self.attn_embedding_dim)

        # 3. 特征级自注意力层
        # 作用：在特征维度 G 上进行交互，计算 G 个特征 Token 之间的相互关系。
        # 注意力机制的嵌入维度必须能被头数整除。
        assert self.attn_embedding_dim % ATTN_NUM_HEADS == 0, "attn_embedding_dim must be divisible by num_heads"
        self.attention = MultiheadAttention(embed_dim=self.attn_embedding_dim,
                                            num_heads=ATTN_NUM_HEADS,
                                            dropout=0.0,  # Actor中通常不使用dropout以避免策略过于随机
                                            batch_first=True)

        # 4. 共享MLP骨干网络
        # 作用：对注意力模块输出的特征进行进一步的非线性变换。
        # ========================= [💥 关键修正 💥] =========================
        # 输入维度必须是注意力层输出展平后的维度: rnn_hidden_size * attn_embedding_dim
        # 这是因为注意力层为每个特征都生成了一个 embedding_dim 的向量。
        mlp_input_dim = self.rnn_hidden_size * self.attn_embedding_dim
        # ===================================================================

        shared_layers_dims = [mlp_input_dim] + ACTOR_PARA.model_layer_dim
        self.shared_network = Sequential()
        for i in range(len(shared_layers_dims) - 1):
            self.shared_network.add_module(f'fc_{i}', Linear(shared_layers_dims[i], shared_layers_dims[i + 1]))
            self.shared_network.add_module(f'LeakyReLU_{i}', LeakyReLU())

        mlp_output_dim = ACTOR_PARA.model_layer_dim[-1]  # MLP 最终输出的特征维度

        # 5. 输出头 (Action Heads)
        self.mu_head = Linear(mlp_output_dim, CONTINUOUS_DIM)  # 连续动作均值头
        # 将对数标准差设为可学习的参数，而不是网络输出，这是一种常见且稳定的做法
        self.log_std_param = torch.nn.Parameter(torch.zeros(1, CONTINUOUS_DIM) * -0.5)
        self.discrete_head = Linear(mlp_output_dim, TOTAL_DISCRETE_LOGITS)  # 离散动作 logits 头

        # --- 初始化与优化器 ---
        self.apply(init_weights)  # 应用自定义的权重初始化
        # 对输出头进行特定的初始化
        torch.nn.init.xavier_uniform_(self.mu_head.weight)
        torch.nn.init.constant_(self.mu_head.bias, 0)
        torch.nn.init.xavier_uniform_(self.discrete_head.weight)
        torch.nn.init.constant_(self.discrete_head.bias, 0)

        # --- 精细化优化器设置 ---
        # 这种设置允许对网络的不同部分使用不同的超参数（如学习率、权重衰减）
        attention_weight_decay = 1e-3  # 对注意力相关层使用更强的权重衰减以进行正则化
    ...
    def forward(self, obs, hidden_state):
        ...
        B, S, D = obs_tensor.shape  # Batch, Sequence Length, Feature Dimension
        G = self.rnn_hidden_size
        N1 = self.attn_embedding_dim

        # --- 阶段一：时序建模 ---
        # 1. GRU 处理: (B, S, D) -> (B, S, G)
        gru_out, new_hidden = self.gru(obs_tensor, hidden_state)

        # --- 阶段二：特征交互 ---
        # 2. 为特征注意力准备: (B, S, G) -> (B*S, G, 1)
        #    将每个时间步的 G 个特征视为 G 个独立的 Token。
        #    B*S 将批次和序列维度合并，以便并行处理所有时间步。
        feat_tokens = gru_out.reshape(B * S, G, 1)

        # 3. 特征嵌入: (B*S, G, 1) -> (B*S, G, N1)
        token_embeds = self.feature_embed_attn(feat_tokens)

        # 4. 特征级自注意力
        #    Query, Key, Value 都是 token_embeds，进行自注意力计算。
        attn_out, _ = self.attention(token_embeds, token_embeds, token_embeds)
        #    残差连接，将原始输入与注意力输出相加，有助于稳定训练。
        token_context = token_embeds + attn_out  # shape: (B*S, G, N1)

        # --- 阶段三：决策 ---
        # 6. 展平以保留所有信息
        #    (B*S, G, N1) -> (B*S, G * N1)
        flattened_context = token_context.contiguous().view(B * S, -1)
        #    恢复序列结构: (B*S, G * N1) -> (B, S, G * N1)
        contextualized_sequence = flattened_context.view(B, S, -1)

        # 7. MLP 加工
        mlp_output = self.shared_network(contextualized_sequence)

        # 如果是单步推理，移除序列维度以获得当前步的特征
...
class Critic_GRU(Module):
    """
    Critic 网络 (价值网络) - 采用与 Actor 相似的结构，但更简化。
    这里的实现是 GRU -> MLP，注意力部分被注释掉了，表明这是一个更简洁的设计选择。
    """
    ...
    def __init__(self, dropout_rate=0.05, weight_decay=1e-4):
        ...
        self.rnn_hidden_size = self.input_dim  # GRU隐藏层大小与输入维度相同

        # --- 模块定义 ---
        # 1. GRU 时序建模层
        self.gru = GRU(self.input_dim, self.rnn_hidden_size, batch_first=True)

        # 注意：此 Critic 版本中，注意力机制相关的层被注释掉了，
        # 数据流直接从 GRU 进入 MLP。
        # # 2. 特征嵌入层 (为 Attention 准备)
        # self.feature_embed_attn = Linear(1, self.attn_embedding_dim)
        # # 3. 特征级自注意力层
        ...
        # --- 初始化与优化器 ---
        self.apply(init_weights)  # 应用自定义权重初始化
        ...
        # default_weight_decay = 1e-4  # 使用局部变量，因为 self.weight_decay 在这里未定义
        attention_params, other_params = [], []
    ...
    def forward(self, obs, hidden_state):
        ...
        is_sequence = obs_tensor.dim() == 3
        if not is_sequence:
            obs_tensor = obs_tensor.unsqueeze(1)  # 增加序列维度

        B, S, D = obs_tensor.shape
        G = self.rnn_hidden_size
        # N1 = self.attn_embedding_dim # 在此版本中未使用

        # 1. GRU
        gru_out, new_hidden = self.gru(obs_tensor, hidden_state)
        # 注意：此处数据直接从 GRU 输出到 MLP，没有经过注意力层。
...
class PPO_continuous(object):
    ...
    def __init__(self, load_able: bool, model_dir_path: str = None, use_rnn: bool = False):
        ...
        if self.use_rnn:
            print("--- 初始化 PPO Agent (使用 [GRU -> Attention -> MLP] 模型) ---")
        ...
        self.gae_lambda = AGENTPARA.lamda  # GAE lambda 参数
        self.ppo_epoch = AGENTPARA.ppo_epoch  # PPO 训练的 epoch 数
        self.total_steps = 0

        # 设置模型保存路径
        self.training_start_time = time.strftime("PPOGRU_Attn_%Y-%m-%d_%H-%M-%S")  # 更新存档文件夹名称
    ...
    def store_experience(self, state, action, probs, value, reward, done, actor_hidden=None, critic_hidden=None):
        """将单步经验存储到 Buffer 中。"""
        # 检查 log_prob 是否包含无效值（NaN 或 Inf），这有助于调试
    ...
    def choose_action(self, state, actor_hidden, critic_hidden, deterministic=False):
        ...
        根据当前状态选择动作。
        :param state: 当前环境状态。
        :param actor_hidden: Actor的GRU隐藏状态。
        :param critic_hidden: Critic的GRU隐藏状态。
        :param deterministic: 如果为 True，则选择最可能的动作（用于评估），否则进行随机采样（用于训练）。
        :return: (给环境的动作, 存入buffer的动作, 动作对数概率, 状态价值, 新的actor隐藏状态, 新的critic隐藏状态)
        """
        ...
        with torch.no_grad():  # 在此块中不计算梯度，因为只是在选择动作，而不是训练
            if self.use_rnn:
                # GRU 模型需要传入隐藏状态
            ...
            # u 是正态分布的原始样本。在 PPO 中，我们存储 u 而不是 tanh(u)
            # 因为在计算 log_prob 时需要使用原始分布。
            u = continuous_base_dist.mean if deterministic else continuous_base_dist.rsample()
            action_cont_tanh = torch.tanh(u)  # tanh(u) 是最终发送给环境的动作的基础
    ...
    def learn(self):
        ...
        # 计算回报 G_t (即 TD-target)，它是优势函数的一个无偏估计
        ...
        for _ in range(self.ppo_epoch):
            ...
            for batch_data in batch_generator:
                ...
                actor_loss.backward()
                torch.nn.utils.clip_grad_norm_(self.Actor.parameters(), max_norm=1.0)  # 梯度裁剪防止梯度爆炸
    ...
    def prep_training_rl(self):
        """将 Actor 和 Critic 网络设置为训练模式。这会启用 Dropout 等层。"""
        self.Actor.train()
        self.Critic.train()

    def prep_eval_rl(self):
        """将 Actor 和 Critic 网络设置为评估模式。这会禁用 Dropout 等层。"""
        self.Actor.eval()
        self.Critic.eval()
