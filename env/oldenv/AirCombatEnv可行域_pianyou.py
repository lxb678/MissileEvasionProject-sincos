...
class AirCombatEnv:
    ...
    def reset(self):
        ...
        self.reward = 0
        self.t_laser = 0
        self.delta_theta, self.delta_phi = 0.0, 0.0

        # self.vT_est = np.array([0, 0, 0])

        self.prev_theta_L = None
        self.prev_phi_L = None
        self.prev_phi_L2 = None
        self.last_valid_theta_dot = 0
        self.last_valid_phi_dot = 0
        self.t = []
        self.Xt = []
        ...
        x_target_theta = 0  # 设定飞机初始俯仰角
        # print("初始飞机俯仰角：{}，飞机机动方式：{}".format(np.rad2deg(x_target_theta), self.target_mode))
        # print("初始飞机俯仰角：{}".format(np.rad2deg(x_target_theta)))
        self.x_target_now = np.array([0, 5000, 0, 250, x_target_theta,0])  # 飞机状态[x, y, z, V, theta, psi]
        self.x_missile_now = np.array([850, 0, 0, x_m, 6000, z_m])  # 导弹状态 [V, theta, psi_c, x, y, z]
        # self.control_input = self.generate_control_input(self.x_target_now, self.n_ty_max, self.target_mode)
        Rx = self.x_target_now[0] - self.x_missile_now[3]
        Ry = self.x_target_now[1] - self.x_missile_now[4]
        Rz = self.x_target_now[2] - self.x_missile_now[5]
        ...
        x_missile_theta = 0  # 导弹初始倾角 :随机产生，处于 0度 和 弹目倾角 之间
        x_missile_psi_c = 0  # 导弹初始偏角 :随机产生，处于 0度 和 弹目偏角 之间
        self.x_missile_now[1] = x_missile_theta
        self.x_missile_now[2] = x_missile_psi_c
        "可行域初始化"
    ...
    def run_one_step(self, dt, release_flare, open_laser):
        ...
        if open_laser == 1:
            ...
            # self.interference_success = self.laser_interference(R_vec, R_rel,self.compute_velocity_vector(self.x_missile_now))
            # self.delta_theta, self.delta_phi = 0.0, 0.0
            self.interference_success, self.delta_theta, self.delta_phi = self.laser_interference_angle_error(R_vec, R_rel,self.compute_velocity_vector(self.x_missile_now),self.target_mode,self.t_laser)
            if self.interference_success:
                self.t_laser += dt
                # if self.t_laser == dt:
                #     self.target_pos_equiv2 = self.x_target_now[:3]
                #     print("self.target_pos_equiv2", self.target_pos_equiv2)
            else:
                self.t_laser = 0.0
        else:
            self.t_laser = 0.0
        ...
        print("t_now, dis,delta_theta, delta_phi",self.t_now, R_rel, np.rad2deg(self.delta_theta), np.rad2deg(self.delta_phi))
        ...
        if I_total > 0:
            ...
            R = np.sqrt(Rx ** 2 + Ry ** 2 + Rz ** 2)
            self.theta_L = np.arcsin(np.clip(Ry / R, -1.0, 1.0)) - self.delta_theta
            self.phi_L = np.arctan2(Rz, Rx) + self.delta_phi
            # theta_L_dot1 = 0.0 if self.prev_theta_L is None else (self.theta_L - self.prev_theta_L) / dt
            # dphi = np.arctan2(np.sin(self.phi_L - self.prev_phi_L),
            #                   np.cos(self.phi_L - self.prev_phi_L)) if self.prev_phi_L else 0.0
            # phi_L_dot1 = dphi / dt
            # print("真的theta_L_dot, phi_L_dot", theta_L_dot1, phi_L_dot1)
            # if self.phi_L <= 0:
            #     self.phi_L -= self.delta_phi
            # else:
            #     self.phi_L += self.delta_phi
            # if self.theta_L <= 0:
            #     self.theta_L -= self.delta_theta
            # else:
            #     self.theta_L += self.delta_theta
            ...
            dphi = np.arctan2(np.sin(self.phi_L - self.prev_phi_L),
                              np.cos(self.phi_L - self.prev_phi_L)) if self.prev_phi_L else 0.0
            phi_L_dot = dphi / dt
            # print("真的theta_L_dot, phi_L_dot", theta_L_dot, phi_L_dot)
            # theta_L_dot += self.delta_theta / dt
            # phi_L_dot += self.delta_phi / dt
            # print("假的的theta_L_dot, phi_L_dot", theta_L_dot, phi_L_dot)

            # if theta_L_dot < 0:
            #     theta_L_dot += self.delta_theta
            # else:
            #     theta_L_dot -= self.delta_theta
            # if phi_L_dot < 0:
            #     phi_L_dot += self.delta_phi
            # else:
            #     phi_L_dot -= self.delta_phi
            # if self.interference_success:
            #     theta_L_dot *= 30
            ...
            # print("加误差phi_L_dot", np.rad2deg(phi_L_dot))
        else:
        ...
        def update_missile(x, theta_L_dot, phi_L_dot, N, ny_max, nz_max):
            ...
            # Rz = zt - z_pos
            # R = np.sqrt(Rx ** 2 + Ry ** 2 + Rz ** 2)
            # theta_t += self.delta_theta
            # psi_t += self.delta_phi
            #
            ...
            # ny = self.N * V * theta_L_dot1 / g + np.cos(theta)
            # nz = -self.N * V * phi_L_dot1 / g
            ...
            # nz = np.clip(nz, -self.nz_max, self.nz_max)
            print("假的ny,nz", ny, nz)
    ...
    def laser_interference(self, R_vec, R_rel, V_missile_vec):
        ...
        if alpha > self.Angle_IR:
            ...
            # 计算激光打在探测器表面焦面功率密度
            I0 = (4 * tau0 * tau1 * P0 * (D0 / d0) ** 2 * np.sqrt(np.cos(alpha))) / (np.pi * R ** 2 * theta ** 2)
    ...
    def laser_interference_angle_error(self, R_vec, R_rel, V_missile_vec, maneuver_type, t_laser):
        """
        激光定向干扰改进版：返回角度误差而非布尔值
        :param R_vec: 相对位置向量
        :param R_rel: 距离 (m)
        :param V_missile_vec: 导弹速度向量
        :param maneuver_type: 目标机动类型 ['turn', 'pitch']
        :param t_laser: 激光持续时间 (s)
        :return: Δθ（倾角误差）, Δφ（偏角误差）
        """
        # 参数设定
        H = 5  # 目标高度 (m)
        W = 16  # 目标宽度 (m)
        ...
        tau0 = 0.8  # 大气透过率
        tau1 = 0.8  # 光学镜头透过率
        Ith = 1*10**-3  # 刚饱和时的激光能量 (W/m^2)
        lambda_ = 10.6  # 激光波长 (um)
        t_response = 3.0  # 响应时间 (s)

        # 入射角
        alpha = np.arccos(np.dot(R_vec, V_missile_vec) / (np.linalg.norm(R_vec) * np.linalg.norm(V_missile_vec)))
        if alpha > self.Angle_IR:
            return False, 0.0, 0.0  # 入射角过大，未能照射导引头
        ...
        if spot_area <  target_area:
            # print("失败")
            return False,0.0, 0.0  # 没有成功干扰导弹导引头

        # 目标半径选择
        target_radius = np.sqrt(target_area / np.pi)

        #光斑与目标半径之差
        delta_r = r_laser - target_radius
        # delta_r = r_laser
        # 计算最大角度误差
        max_angle_error = np.arctan(delta_r / f) * 5
        # max_angle_error = np.deg2rad(1)
        # angle_error = max_angle_error
        angle_error = max_angle_error * (1 - np.exp(-t_laser / t_response)) * 1

        # 增长型误差模型
        # max_error = np.deg2rad(30)
        # k = 0.1
        # angle_error = max_error * (1 - np.exp(-k * t_laser))

        # angle_error = max_angle_error * 1
        # 输出扰动角度
        delta_theta = angle_error if maneuver_type in [3, 4] else 0.0
        delta_phi = angle_error if maneuver_type in [1, 2] else 0.0

        return True, delta_theta, delta_phi
