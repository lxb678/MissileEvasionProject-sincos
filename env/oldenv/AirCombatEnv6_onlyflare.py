...
class AirCombatEnv:
    ...
    def render(self):
        ...
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.plot(self.Y[:, 3], self.Y[:, 5], self.Y[:, 4], 'b-', label='Missile trajectory')
        ax.plot(self.Xt[:, 0], self.Xt[:, 2], self.Xt[:, 1], 'r--', label='Target trajectory')
        ax.scatter(self.Y[0, 3], self.Y[0, 5], self.Y[0, 4], color='g', label='Missile launch point')
        ax.scatter(self.Y[self.idx_min, 3], self.Y[self.idx_min, 5], self.Y[self.idx_min, 4],
                   color='m', label='Closest point')

        for i, flare in enumerate(self.flare_manager.flares):
            traj = np.array(flare.history)
            if traj.shape[0] > 0:
                ax.plot(traj[:, 0], traj[:, 2], traj[:, 1], color='orange', linewidth=1,
                        label='Infrared flare' if i == 0 else "")
        ...
        ax.invert_yaxis()
        ax.legend()
        ax.set_title('infrared flare simulation')
        ax.grid()
        ax.view_init(elev=20, azim=-150)
        plt.show()
    ...
    def run_one_step(self, dt, release_flare, open_laser):
        ...
        # print("self.x_target_next", self.x_target_next)
