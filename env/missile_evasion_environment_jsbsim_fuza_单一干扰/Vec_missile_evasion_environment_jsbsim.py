...
class AirCombatEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None) -> tuple:
        ...
        if self.maneuver_type == 'dive':
            # 俯冲：目标设为低空
            self.target_height = 1000.0
        elif self.maneuver_type == 'climb':
            # 爬升：目标设为高空
            self.target_height = 12000.0
        else:
        ...
        # 飞机初始状态
        aircraft_pitch = np.deg2rad(np.random.uniform(-10, 10))  # 初始平飞或微俯仰
        ...
        # 添加导弹发射误差
        initial_heading_error_deg = 20.0  # 稍微减小误差，保证威胁
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        aircraft_obs = [
            ...
            2 * ((self.aircraft.pos[1] - 500) / 14500) - 1,  # Alt
