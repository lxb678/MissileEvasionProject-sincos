...
class AirCombatEnv:
    def __init__(self):
        ...
        self.n_ty_max = 5  # 飞机过载

        self.flare_manager = FlareManager()
        # self.flare = Flare()

        self.M_D_max = 30000  # 导引头最大距离
    ...
    def reset(self):
        ...
        self.x_target_now = np.array([0, np.random.uniform(3000, 5000), 0, np.random.uniform(200, 510), x_target_theta, 0])  # 飞机状态[x, y, z, V, theta, psi]
        self.x_missile_now = np.array([np.random.uniform(640,750), 0, 0, x_m, np.random.uniform(3000,5000), z_m])  # 导弹状态 [V, theta, psi_c, x, y, z]
        ...
        # # print("初始飞机俯仰角：{}，飞机机动方式：{}".format(np.rad2deg(x_target_theta), self.target_mode))
        # # print("初始飞机俯仰角：{}".format(np.rad2deg(x_target_theta)))
        # self.x_target_now = np.array([0, np.random.uniform(3000, 5000), 0, np.random.uniform(200, 510), x_target_theta,
        #                               np.deg2rad(20)])  # 飞机状态[x, y, z, V, theta, psi]
        # self.x_missile_now = np.array([np.random.uniform(640, 750), 0, 0, x_m, np.random.uniform(3000, 5000),
        #                                z_m])  # 导弹状态 [V, theta, psi_c, x, y, z]
        # self.x_missile_now[3] = -1000
        # self.x_missile_now[4] = 3600
        ...
        # self.x_missile_now[0] = 850
        # self.x_missile_now[1] = 0
        # self.x_missile_now[2] = -2.792526
        # self.x_missile_now[3] = 11493
        # self.x_missile_now[4] = 9984
        # self.x_missile_now[5] = -8260
        # self.x_target_now[0] = 0
        # self.x_target_now[1] = 8000
        # self.x_target_now[2] = 0
        # self.x_target_now[3] = 340
        # self.x_target_now[4] = 0
        # self.x_target_now[5] = -0.809
        # R = 17372
        # angle_1 = 5.6607
        # angle_2 = 0.139
        # self.target_mode = 1
        ...
        if o_di[0] < np.pi and np.abs(Ry) <= 1000:
            ...
            self.target_mode = 4
        # self.target_mode = np.random.randint(3, 5)  # 1=左转，2=右转，3=爬升，4=俯冲
        # self.target_mode = 3
        self.control_input = self.generate_control_input(self.x_target_now, self.n_ty_max, self.target_mode)
    ...
    def step(self, action):
        ...
        if R_rel < self.R_switch:
            for _ in range(int(self.dt / self.dt_small)):
        ...
        #     print(f">>> 仿真达到60s,成功逃离! R = {self.R_mt_now:.2f} m, V_rel = {self.V_rel_now:.2f} m/s")
        #     self.done = True
        ...
        if self.done:
            ...
            reward_4 = 0
            # reward_4 = self.compute_dense_reward_4(action[0]) / 10.0
        else:
            ...
            reward_4 = self.compute_dense_reward_4(action[0]) * 1.3 / 5.0

            # print(self.compute_dense_reward_4(action[0]))
            # print(self.reward)
            self.reward /= 5.0
    ...
    def render2(self, interval=0.05):
        # ========================= 初始化图形 =========================
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.set_xlabel('X / m')
        ...
        # 初始化轨迹线条
        missile_line, = ax.plot([], [], [], 'b-', label='导弹轨迹')
        target_line, = ax.plot([], [], [], 'r--', label='目标轨迹')
        missile_start = ax.scatter(self.Y[0, 3], self.Y[0, 5], self.Y[0, 4], color='g', label='导弹起点')
        missile_closest = ax.scatter(self.Y[self.idx_min, 3], self.Y[self.idx_min, 5], self.Y[self.idx_min, 4],
                                     color='m', label='最近点')
        ...
        for i, flare in enumerate(self.flare_manager.flares):
            line, = ax.plot([], [], [], color='orange', linewidth=1,
                            label='红外诱饵弹' if i == 0 else "")
            flare_lines.append(line)

        ax.legend()

        # 动态绘制
        for t in range(len(self.Y)):
            missile_line.set_data(self.Y[:t + 1, 3], self.Y[:t + 1, 5])
            missile_line.set_3d_properties(self.Y[:t + 1, 4])

            target_line.set_data(self.Xt[:t + 1, 0], self.Xt[:t + 1, 2])
            target_line.set_3d_properties(self.Xt[:t + 1, 1])

            # 诱饵弹轨迹更新
            for i, flare in enumerate(self.flare_manager.flares):
                traj = np.array(flare.history)
                if traj.shape[0] > t:
                    flare_lines[i].set_data(traj[:t + 1, 0], traj[:t + 1, 2])
                    flare_lines[i].set_3d_properties(traj[:t + 1, 1])
    ...
    def run_one_step(self, dt, release_flare, open_laser):
        ...
        lock_aircraft, *_ = self.check_seeker_lock(self.x_missile_now, self.x_target_now,
                                                   ...
                                                   self.t_now, self.M_D_max, self.Angle_IR, self.omega_max, self.T_max)
        ...
        if I_total > 0:
            ...
            # print("self.theta_L，self.phi_L", np.rad2deg(self.theta_L),np.rad2deg(self.phi_L))
        ...
        def update_missile(x, theta_L_dot, phi_L_dot, N, ny_max, nz_max):
            ...
            # ny =   self.N * np.abs(Rdot) * theta_L_dot / g
            # nz =   -self.N * np.abs(Rdot) * phi_L_dot / g

            # === 1. 过载计算 ===
            # phi_L_dot = np.clip(phi_L_dot, -0.01, 0.01)
            ny = self.N * V * theta_L_dot / g
            # if np.rad2deg(theta) > 60:
            #     # nz = np.clip(self.N * V * theta_L_dot / g, -0.5, 0.5)
            #     nz = 0
            # else:
            nz =  -self.N * V * phi_L_dot / g
            ny =  self.N * V * theta_L_dot / g + np.cos(theta)
            # nz =  -self.N * V * phi_L_dot / g * np.cos(theta)
            # if np.abs(psi_c) > np.pi / 2:
            #     ny = -self.N * V * theta_L_dot / g
            #     nz = self.N * V * phi_L_dot / g
            ...
            if n_total_cmd > n_max:
                ...
                ny *= scale
                nz *= scale
            # ny = np.clip(ny, -self.ny_max, self.ny_max)
            # nz = np.clip(nz, -30, 30)

            # print("ny", ny)
            # print("nz", nz)
            print("V", V)
            # print("theta_L_dot", theta_L_dot)
            # print("phi_L_dot", phi_L_dot)
            # print("theta", theta)
            # print("n_total_cmd", n_total_cmd)

            # dphi = g / V * (nz / np.cos(theta))
            # dtheta = g / V * (ny - np.cos(theta))
            # print("dtheta", dtheta)
            ...
            def get_Cx2(Ma):
                if 0 < Ma <= 0.9:
                    cd = 0.16
                if 0.9 < Ma <= 1.1:
                    cd = 0.16 + 0.29 * (Ma - 0.9) / 0.2
                if 1.1 < Ma <= 3:
                    cd = 0.45 - 0.25 * (Ma - 1.1) / 1.9
                else:
                    cd = 0.2
                return cd
            ...
            q = euler_to_quat(psi_c, theta) #俯仰和偏航反了？
            ...
            # a_gravity = np.array([0.0, 0.0, 0.0])
            # === 6. 合加速度 ===
            a_total = a_overload + a_gravity + a_drag
            # print("a_overload_body",a_overload_body)
            # print("a_overload",a_overload)
            # print("a_total",a_total)

            # === 7. 速度更新 ===
            v_next = v_now + a_total * dt

            print("v_next",v_next)
            V_next = np.linalg.norm(v_next)

            # # === 8. 姿态更新 ===
            theta_next = np.arcsin(np.clip(v_next[1] / V_next, -1.0, 1.0))
            # # theta_next = np.arctan2(v_next[1], np.sqrt(v_next[0] ** 2 + v_next[2] ** 2))
            psi_c_next = np.arctan2(-v_next[2], v_next[0])  # 注意符号，与模型一致
            # theta_next = np.clip(theta_next, -np.deg2rad(80), np.deg2rad(80))  # 限制 theta 在 [-π, π] 范围内
            # theta_next = theta + dtheta * dt
            # psi_c_next = psi_c + dphi * dt
            # print("theta_next",np.rad2deg(theta_next))
            # print("psi_c_next",np.rad2deg(psi_c_next))
            ...
            # if psi_c_next < -np.pi:
            #     psi_c_next += 2 * np.pi
            # v_next = V_next * np.array([np.cos(theta_next) * np.cos(psi_c_next), np.sin(theta_next), -np.cos(theta_next) * np.sin(psi_c_next)])
        ...
        if R_rel < 200:
            ...
            if t1 <= t_star <= t2:
                ...
                #                                                   self.x_missile_next, self.x_target_next, self.t_now - dt,
                #                                                   self.t_now)
        ...
        if (not lock_aircraft or self.interference_success) and \
    ...
    def check_seeker_lock(self, x_missile, x_target, V_missile_vec, V_target_vec, t_now,
        ...
        # print("V_missile_vec", V_missile_vec)
        ...
        # print("Flag_D and Flag_A and Flag_omega and Flag_T", Flag_D, Flag_A, Flag_omega, Flag_T)
    ...
    def calculate_miss_distance(self, x_missile_now, x_target_now, x_missile_next, x_target_next, t_now, t_next):
        ...
        if denominator == 0:
            ...
            t_star = t1 - (M1_T1.dot(T1_T2 - M1_M2) * delta_t) / denominator

        print(t1,t_star,t2)
        # 计算导弹脱靶量
        d_miss = np.linalg.norm(M1_T1 + (t_star - t1) * (T1_T2 - M1_M2) / delta_t)
        print(d_miss,np.linalg.norm(M1_T1),np.linalg.norm(M2_T2))
