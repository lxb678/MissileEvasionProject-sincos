...
class AirCombatEnv:
    ...
    def reset(self) -> np.ndarray:
        ...
        self.aircraft = Aircraft(initial_aircraft_state)
    ...
    def step(self, action: list) -> tuple:
        ...
        """
        nx, nz, p_cmd, release_flare_cmd = action[0], action[1], action[2], action[3] > 0.5

        # --- 1. 保存决策步开始前的状态，用于奖励计算 ---
        self.prev_aircraft_state = self.aircraft.state_vector.copy()
        self.prev_missile_state = self.missile.state_vector.copy()
        ...
        # 确定步长和循环次数
        # (这里的参数 dt_small, dt_flare, dt_normal, R_switch 应在 __init__ 中定义)
        # 根据情况选择不同的内部循环
        if R_rel_start < self.R_switch:
            num_steps = int(round(self.dt_dec / self.dt_small))
            step_dt = self.dt_small
        elif release_flare_cmd or self.flare_manager.schedule:
            num_steps = int(round(self.dt_dec / self.dt_flare))
            step_dt = self.dt_flare
        else:
            num_steps = int(round(self.dt_dec / self.dt_normal))
            step_dt = self.dt_normal

        # 执行内部物理循环
        for i in range(num_steps):
            # 只有在循环的第一步才考虑投放指令
            current_flare_release = 1.0 if (i == 0 and release_flare_cmd) else 0.0

            self.run_one_step(step_dt, nx, nz, p_cmd, current_flare_release)

            # 如果在内部循环中回合已经结束（例如被引信引爆），则立即跳出
            if self.done:
                break
    ...
    def run_one_step(self, dt, nx, nz, p_cmd, release_flare):
        """
        (修正版) 执行单个物理时间步的更新，并包含所有终止条件检查。
        """
        # --- 1. (新时序) 计算下一时刻的状态，但不立即更新对象 ---
        # a) 飞机：计算下一时刻的状态
        action = [nx, nz, p_cmd, release_flare]
        aircraft_state_next_raw = self.aircraft._aircraft_dynamics(self.aircraft.state_vector, dt, nx, nz, p_cmd)
        # --- (中文) 核心修正：在这里应用飞行包线限制 ---
        aircraft_state_next = aircraft_state_next_raw.copy()  # 创建一个副本以避免修改原始输出
        aircraft_state_next[3] = np.clip(
            aircraft_state_next[3],
            self.aircraft.min_velocity_ms,
            self.aircraft.max_velocity_ms
        )
        # b) 诱饵弹：根据【当前】飞机状态，更新诱饵弹
        if release_flare == 1 and self.o_ir > 0:
            self.o_ir -= self.flare_manager.flare_per_group
    ...
    def _calculate_final_miss_distance(self):
        ...
        print(f">>> 最终评估：{'命中' if is_hit else '未命中'}，脱靶量为：{self.miss_distance:.2f} m")
