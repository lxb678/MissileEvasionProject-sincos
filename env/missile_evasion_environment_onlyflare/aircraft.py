...
class Aircraft:
    ...
    def update(self, dt: float, action: list):
        ...
        # 解包新的 action 格式
        if len(action) != 4:
            raise ValueError(f"Action 列表需要4个元素 [nx, nz, roll_command, roll_mode], 但收到了 {len(action)} 个。")
        # 注意: 在您的模型中，action[0]是nx(推力)，action[1]是nz(过载)，action[2]是p_cmd(滚转速率)
        nx_cmd, nz_cmd, roll_command, roll_mode = action

        # 调用独立的动力学函数来计算新状态
        self.state = self._aircraft_dynamics(self.state, dt, nx_cmd, nz_cmd, roll_command, roll_mode)

        # # 限制俯仰角和速度
        max_pitch_rad = np.deg2rad(60.0)
        self.state[4] = np.clip(self.state[4], -max_pitch_rad, max_pitch_rad)
    ...
    # --- 核心动力学模型 (作为私有方法) ---
    def _aircraft_dynamics(self, state: np.ndarray, dt: float, nx: float, nz: float, roll_command: float, roll_mode: int) -> np.ndarray:
        """
        飞机的核心动力学积分器。这是一个纯函数，不修改类自身的状态。
        飞机的核心动力学积分器。
    通过 roll_mode 标志明确区分滚转角和滚转速率指令。

    Args:
        ...
        roll_command (float): 滚转控制指令值。
        roll_mode (int): 滚转控制模式 (0: 目标滚转角, 1: 滚转速率指令)。
        """
        # ================== 坐标系定义 ==================
        ...
        # === 2. 滚转动力学 (根据 roll_mode 明确选择) ===
        if roll_mode == 0:
            # --- 模式 0: 输入是目标滚转角 ---
            # # === 2. <<< MODIFIED >>> 滚转动力学 (从目标滚转角生成滚转速率指令) ===
            # # The function now receives 'target_roll_angle' instead of 'p_cmd'
            # # Our goal is to calculate the p_cmd needed to reach that angle.
            # # a) 定义P控制器增益 (这是一个可调参数，5.0是一个不错的起点)
            Kp_roll = 5.0
            phi_cmd = roll_command
            target_roll_angle = phi_cmd
            # b) 从状态向量中获取当前的滚转角 (phi)
            current_roll_angle = phi

            # c) 计算滚转角误差 (目标 - 当前)
            roll_angle_error = target_roll_angle - current_roll_angle

            # d) (重要!) 处理角度环绕问题，确保飞机走最短路径滚转
            # 例如，从 -170度 到 +170度，应该滚转 -20度，而不是 +340度
            roll_angle_error = np.arctan2(np.sin(roll_angle_error), np.cos(roll_angle_error))
            # e) 计算指令滚转速率 p_cmd
            p_cmd = Kp_roll * roll_angle_error
            # f) (您的原始代码) 现在使用我们计算出的 p_cmd 来更新实际滚转速率 p_real
            ...
            raise ValueError(f"无效的滚转模式标志: {roll_mode}. 必须是 0 或 1。")
