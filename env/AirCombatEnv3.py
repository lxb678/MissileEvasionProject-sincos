...
class AirCombatEnv:
    ...
    def reset(self):
        ...
        self.x_target_now = np.array([0, np.random.uniform(3000,5000), 0, np.random.uniform(200,510), 0, 0])  # 飞机状态[x, y, z, V, theta, psi]
    ...
    def step(self, action):
        ...
        if self.t_now >= self.t_end and not self.done:
            print(f">>> 仿真达到30s,成功逃离! R = {self.R_mt_now:.2f} m, V_rel = {self.V_rel_now:.2f} m/s")
            self.done = True
        ...
        if self.done:
            ...
            self.reward += self.compute_dense_reward_3()
            self.reward += self.compute_dense_reward_4(release_flare)
            # self.reward /=10

        return self.observation, self.reward, self.done, 1, 1
    ...
    def compute_dense_reward_3(self):
        """激光定向干扰导致导引头失锁，给予奖励"""
