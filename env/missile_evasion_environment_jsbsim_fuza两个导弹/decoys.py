...
class Flare:
    ...
    def __init__(self, position: np.ndarray, velocity: np.ndarray, release_time: float,
                 ...
                 I_max= 5000, t1=0.5, t2=3.5, t3=5.0):
        """
        初始化单个诱饵弹。
        """
...
class FlareManager:
    ...
    def __init__(self, flare_per_group=1, interval=0.1, release_speed=40):
    ...
    def update(self, t: float, dt: float, aircraft: Aircraft):
        ...
        if newly_scheduled_times:
            ...
            # 【核心修正3】：将 FRD 机体坐标系直接映射到 NUE (北-天-东) 世界坐标系
            R_b_to_n = np.array([
