...
class AirCombatEnv(gym.Env):
    ...
    def __init__(self, tacview_enabled=False):
        ...
        # 5个连续动作: throttle, elevator, aileron, rudder, release_flare
        # throttle: [0, 1], others: [-1, 1], flare: [0, 1] (用伯努利分布采样后)
    ...
    def step(self, action: list) -> tuple:
        ...
        """
        # 解包新的动作向量
        throttle_cmd, elevator_cmd, aileron_cmd, rudder_cmd, release_flare_cmd_raw = action
        release_flare_cmd = release_flare_cmd_raw > 0.5
        ...
        for i in range(num_steps):
            ...
            self.run_one_step(step_dt, aircraft_action, current_flare_release)
    ...
    def run_one_step(self, dt, aircraft_action, release_flare):
        """
        (修正版) 执行单个物理时间步的更新，并包含所有终止条件检查。
        """
        # <<< 更改 >>> aircraft_action 现在是 [油门, 升降舵, 副翼, 方向舵]
