# 文件: aircraft.py (建议重命名为 AircraftPointMass_3DOF.py)

import numpy as np


class AircraftPointMass:
    """
    ...
    def _aircraft_dynamics(self, state: np.ndarray, dt: float, nx: float, nz: float, p_cmd: float) -> np.ndarray:
        ...
        #控制量为滚转角
        phi_cmd = p_cmd #为了简单，认为p_cmd就是滚转角指令
        p_body_cmd = (1 / self.tau_roll) * (phi_cmd - phi)
        p_body = np.clip(p_body_cmd, -np.deg2rad(240), np.deg2rad(240))  # 使用实际滚转速率并加以限制
        p_real_new = p_body
        # # === 2. 滚转动力学 (核心修改部分) ===
        # # ------------------- START OF MODIFICATION -------------------
        # # 目标: 根据目标滚转角 phi_cmd (rad) 和当前滚转角 phi (rad)
        # #       生成一个内部的滚转速率指令 p_internal_cmd (rad/s)。
        #
        # # P控制器增益，决定了飞机响应目标滚转角指令的积极性
        # Kp_roll_angle = 8.0
        #
        # # 计算滚转角误差 (需要处理角度环绕问题，确保走最短路径)
        # roll_angle_error = phi_cmd - phi
        # roll_angle_error = (roll_angle_error + np.pi) % (2 * np.pi) - np.pi
        #
        # # 生成内部滚转速率指令
        # p_internal_cmd = Kp_roll_angle * roll_angle_error  # 单位: rad/s
        #
        # # 将内部指令传递给原始的滚转速率响应模型
        # p_dot = (1 / self.tau_roll) * (p_internal_cmd - p_real)
        # p_real_new = p_real + p_dot * dt
        #
        # # 限制实际滚转速率
        # MAX_ROLL_RATE_RAD = np.deg2rad(240)
        # p_body = np.clip(p_real_new, -MAX_ROLL_RATE_RAD, MAX_ROLL_RATE_RAD)
