...
class AirCombatEnv:
    ...
    def run_one_step(self, dt, release_flare, open_laser):
        ...
        if open_laser == 1:
            ...
            self.interference_success, delta_theta, delta_phi = self.laser_interference_angle_error(R_vec, R_rel,
            ...
            self.t_laser = 0.0
            delta_theta, delta_phi = 0.0, 0.0

        print("t_now, dis,delta_theta, delta_phi",self.t_now, R_rel, np.rad2deg(delta_theta), np.rad2deg(delta_phi))
        ...
        if I_total > 0:
            ...
            R = np.sqrt(Rx ** 2 + Ry ** 2 + Rz ** 2)
            self.theta_L = np.arcsin(np.clip(Ry / R, -1.0, 1.0)) - delta_theta
            self.phi_L = np.arctan2(Rz, Rx) - delta_phi
    ...
    def laser_interference_angle_error(self, R_vec, R_rel, V_missile_vec, maneuver_type, t_laser):
        ...
        delta_r = r_laser
        # 计算最大角度误差
        max_angle_error = np.arctan(delta_r / f) * 100
